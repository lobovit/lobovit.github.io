...
No [[/post/histcomp01][artigo anterior]] vimos que informação pode ser codificada em padrões binários — pontos e traços no Morse, pontos em relevo no Braille, bits 0 e 1 no ASCII. Mas codificar é só metade da história. A pergunta agora é: como construir uma máquina que /processa/ fisicamente esses 0s e 1s?
...
Toda matéria é feita de [[https://pt.wikipedia.org/wiki/Átomo][átomos]], e cada átomo é como um sistema solar em miniatura: um núcleo de /[[https://pt.wikipedia.org/wiki/Nêutron][nêutrons]]/ (sem carga) e /[[https://pt.wikipedia.org/wiki/Próton][prótons]]/ (carga positiva), orbitado por /[[https://pt.wikipedia.org/wiki/Elétron][elétrons]]/ (carga negativa). Todos os elétrons do universo são idênticos e indistinguíveis entre si.
...
![[/post/images/hist/corrente01.png][corrente01]]

Para entender esse fluxo, usamos a analogia clássica da água encanada:

/ /[[https://pt.wikipedia.org/wiki/Tensão_elétrica][Tensão]] (Volts - E):* É a "pressão" da água. Uma bateria tem o potencial de realizar trabalho, como uma caixa d'água no alto de um prédio.
/ /[[https://pt.wikipedia.org/wiki/Corrente_elétrica][Corrente]] (Amperes - I):* É a quantidade de água (elétrons) fluindo pelo cano.
/ /[[https://pt.wikipedia.org/wiki/Resistência_elétrica][Resistência]] (Ohms - R):* É o "estreitamento" do cano. Quanto mais fino ou mais longo o cano, maior a resistência, menos água passa.

A famosa [[https://pt.wikipedia.org/wiki/Lei_de_Ohm][Lei de Ohm]] conecta essas grandezas:
...
Embora [[https://pt.wikipedia.org/wiki/Thomas_Edison][Thomas Edison]] costume levar o crédito, foi o britânico [[https://pt.wikipedia.org/wiki/Joseph_Swan][Joseph Swan]] quem de fato inventou a lâmpada incandescente prática. Dentro do bulbo de vidro há um fino /[[https://pt.wikipedia.org/wiki/Tungstênio][filamento de tungstênio]]/ que brilha quando aquecido pela passagem de corrente elétrica. O bulbo é preenchido com um /gás inerte/ ([[https://pt.wikipedia.org/wiki/Árgon][argônio]] ou [[https://pt.wikipedia.org/wiki/Nitrogénio][nitrogênio]]) para evitar que o filamento entre em combustão e queime instantaneamente ao entrar em contato com o oxigênio.
...
A potência consumida por um componente é dada pela /[[https://pt.wikipedia.org/wiki/James_Watt][Lei de Watt]]/ (homenagem a [[https://pt.wikipedia.org/wiki/James_Watt][James Watt]], o inventor da [[https://pt.wikipedia.org/wiki/Máquina_a_vapor][máquina a vapor]]):
...
Se você conectar o fio positivo diretamente ao negativo sem nenhuma resistência no caminho (resistência ≈ 0), a corrente dispara para valores altíssimos (I = E / R, e R é quase zero). Isso é um /curto-circuito/: o fio esquenta, pode brilhar e até derreter. É por isso que /[[https://pt.wikipedia.org/wiki/Fusível][fusíveis]]/ e /[[https://pt.wikipedia.org/wiki/Disjuntor][disjuntores]]/ existem, para interromper o circuito antes que o fio vire um incêndio.
...
Mas há um problema: fios longos têm resistência. Um fio de cobre de [[https://pt.wikipedia.org/wiki/American_wire_gauge][bitola 20 AWG]] (típico de instalações elétricas) tem cerca de 10 ohms de resistência a cada 300 metros. Se a casa do vizinho estiver a 1,5 km, o fio terá mais de /100 ohms/ de resistência. Pela Lei de Ohm (I = 3V / 100Ω = 0,03A), uma bateria de 3V mal conseguiria gerar corrente suficiente para fazer uma lâmpada de 4 ohms brilhar. Por isso o telégrafo não usava lâmpadas: em vez disso, usava /[[https://pt.wikipedia.org/wiki/Eletroímã][eletroímãs]]/ que emitiam "cliques" audíveis, dispositivos que exigem muito menos corrente para operar.
...
Mesmo com 2 fios, a resistência em longas distâncias continua sendo um problema. Foi exatamente esse o desafio enfrentado na criação do [[https://pt.wikipedia.org/wiki/Telégrafo][telégrafo]]. A solução genial foi o /[[https://pt.wikipedia.org/wiki/Relé][Relé]]/ (Relay).

![[/post/images/hist/rele01.png][corrente01]]
...
Com este sistema de 2 fios e relés, ambas as casas podem /enviar e receber/ mensagens simultaneamente pela mesma infraestrutura física compartilhada. Cada lado tem seu próprio eletroímã receptor que emite cliques quando a outra parte fecha seu interruptor. É o início da /comunicação bidirecional/ ([[https://pt.wikipedia.org/wiki/Duplex_(telecomunicações][/full-duplex/]])), o mesmo princípio que permite que você baixe um arquivo enquanto envia um email pelo mesmo cabo de rede.
...
Em 1854, o matemático inglês [[https://pt.wikipedia.org/wiki/George_Boole][George Boole]] publicou /Uma Investigação das Leis do Pensamento/. Seu objetivo não tinha nada a ver com computadores ou circuitos elétricos — a eletricidade mal era utilizável na época. Boole queria capturar as /leis do raciocínio humano/ em forma matemática.
...
Combinando um inversor (NOT) com uma porta AND ou OR, obtemos as portas /NAND/ e /NOR/, regidas pelas /[[https://pt.wikipedia.org/wiki/Leis_de_De_Morgan][Leis de Morgan]]/:
...
No [[/post/histcomp01][primeiro artigo]], vimos que qualquer informação pode ser reduzida a sequências de 0s e 1s — Morse, Braille, ASCII, todos são códigos binários. A pergunta natural era: como uma máquina /processa/ esses símbolos?
...
/Fonte:/ [[https://a.co/d/0a3DsSsn][Code: The Hidden Language of Computer Hardware and Software]], 2ª ed., Charles Petzold
