...
A maioria dos sistemas antigos, como os numerais romanos, era péssima para matemática avançada. Experimente multiplicar ~MCMLIII~ por ~XIV~. O sistema que usamos hoje, o /[[https://pt.wikipedia.org/wiki/Sistema_de_numeração_hindu-arábico][Hindu-Arábico]]/, revolucionou a matemática por três motivos:
...
A palavra /[[https://pt.wikipedia.org/wiki/Bit][bit]]/ foi cunhada pelo matemático [[https://en.wikipedia.org/wiki/John_Tukey][John Tukey]] na década de 1940 como uma contração de /binary digit/ (dígito binário).
...
À medida que os computadores evoluíram, organizar os bits individualmente tornou-se caótico. A indústria começou a agrupar bits no que chamamos de /Word/ (Palavra). O tamanho ideal estabelecido para a computação geral foi o agrupamento de 8 bits: o /[[https://pt.wikipedia.org/wiki/Byte][Byte]]/.
...
A solução? O /[[https://pt.wikipedia.org/wiki/Sistema_hexadecimal][Hexadecimal]]/ (base 16). Cada dígito hexadecimal mapeia perfeitamente um grupo de 4 bits (um /[[https://en.wikipedia.org/wiki/Nibble][nibble]]/). Dois dígitos hexadecimais formam exatamente 1 Byte.
