...
Neste artigo, vamos explorar como construir uma aplicação [[https://pt.wikipedia.org/wiki/Geração_aumentada_por_recuperação][RAG (Retrieval-Augmented Generation)]] completa do zero usando [[https://clojure.org/][Clojure]]. Vamos mergulhar em uma implementação prática que combina processamento de texto, busca semântica e geração de respostas com LLMs locais. Se você está interessado em melhorar a precisão e relevância das respostas dos seus modelos de linguagem com informações atualizadas, este guia é para você!

** Fundamentos do RAG

*** O que é RAG?

Os Modelos de Linguagem de Grande Escala ([[https://en.wikipedia.org/wiki/Large_language_model][LLMs]]), como o [[https://openai.com/api/][GPT]], [[https://openai.com/api/][ChatGPT]] e outros, revolucionaram a forma como interagimos com a inteligência artificial. Eles são capazes de gerar textos coerentes, responder perguntas complexas e até mesmo criar conteúdo criativo. No entanto, esses modelos possuem uma limitação fundamental: seu conhecimento é "congelado" no tempo.
...
#+BEGIN_QUOTE
Confiar exclusivamente em um [[https://en.wikipedia.org/wiki/Large_language_model][LLM "puro"]] nesses contextos resultará em respostas desatualizadas, potencialmente imprecisas e, consequentemente, em uma experiência do usuário comprometida. A eficácia da aplicação é diretamente afetada.
#+END_QUOTE
...
1. /Conexão com uma base de dados atual:/ Em vez de depender apenas do conhecimento estático adquirido durante seu treinamento (que pode se tornar obsoleto rapidamente), o [[https://en.wikipedia.org/wiki/Large_language_model][LLM]] ganha acesso a uma fonte de informações dinâmica e constantemente atualizada. Isso pode ser uma base de dados de notícias, um repositório de documentos corporativos, uma coleção de artigos científicos, ou qualquer outra fonte relevante para a tarefa em questão.
2. /Recuperação em fontes externas:/ O [[https://en.wikipedia.org/wiki/Large_language_model][LLM]] não está mais limitado a "lembrar" de informações. Ele adquire a capacidade de "procurar" ativamente por dados relevantes para responder a uma pergunta ou gerar um texto em fontes externas ao modelo, que podem ser estáticas (documentos corporativos) ou atualizadas em tempo real (APIs, bancos de dados dinâmicos). Isso é semelhante a como nós, humanos, usamos mecanismos de busca para encontrar informações que não temos memorizadas. O [[https://en.wikipedia.org/wiki/Large_language_model][LLM]], equipado com RAG, pode formular consultas, analisar os resultados e selecionar as informações mais pertinentes.
3. /Combinação de conhecimento base com dados novos:/ Este é o ponto crucial que diferencia o [[https://pt.wikipedia.org/wiki/Geração_aumentada_por_recuperação][RAG]] de uma simples busca em uma base de dados. O [[https://en.wikipedia.org/wiki/Large_language_model][LLM]] não apenas recupera informações, mas também as integra ao seu conhecimento pré-existente. Ele usa sua capacidade de raciocínio e compreensão para contextualizar os novos dados, identificar contradições, e formular respostas coerentes e informadas.
...
Segundo um [[https://arxiv.org/abs/2309.01066][whitepaper recente dos pesquisadores do Google]], existem várias técnicas para turbinar o desempenho dos [[https://en.wikipedia.org/wiki/Large_language_model][LLMs]], e o RAG é uma das mais promissoras. Isso ocorre porque o RAG aborda algumas das limitações fundamentais desses modelos:

O RAG resolve vários problemas de uma vez só: diminui aquelas "viagens" dos [[https://en.wikipedia.org/wiki/Large_language_model][LLMs]] quando inventam respostas (as famosas alucinações), mantém tudo atualizado em vez de ficar preso no passado, deixa as respostas mais transparentes porque você sabe de onde veio a informação, e ainda melhora o desempenho do modelo quando ele precisa lidar com documentos ou dados específicos da sua empresa. É como dar ao modelo um Google particular que ele pode consultar antes de responder!

#+BEGIN_QUOTE
O RAG representa um avanço significativo na evolução dos [[https://en.wikipedia.org/wiki/Large_language_model][LLMs]], permitindo que eles se tornem ferramentas mais confiáveis, precisas e úteis para uma ampla gama de aplicações. Ele transforma o [[https://en.wikipedia.org/wiki/Large_language_model][LLM]] de um "sabe-tudo" desatualizado em um pesquisador ágil e bem-informado, capaz de combinar conhecimento profundo com informações atualizadas em tempo real.
#+END_QUOTE

*** Por que o DeepSeek R1?

Ele trabalha muito bem com documentação técnica, o que é perfeito para nosso sistema [[https://pt.wikipedia.org/wiki/Geração_aumentada_por_recuperação][RAG]] focado em docs técnicas. O DeepSeek R1 consegue equilibrar qualidade e velocidade melhor que outros modelos do Ollama, rodando na sua máquina sem ficar alucinando com respostas que não fazem sentido.

O modelo também se dá super bem com várias linguagens de programação, incluindo [[https://clojure.org/][Clojure]], então ele responde numa boa sobre implementações técnicas e documentação de código. E o melhor: mesmo quando você joga informações pela metade ou todas bagunçadas, ele ainda consegue manter o contexto e dar respostas que fazem sentido. Por isso ele é perfeito para o que estamos construindo!
...
Pré-requisitos:
- [[https://clojure.org/guides/getting_started][Clojure]]: Linguagem de programação funcional que vamos usar para construir a aplicação
- [[https://leiningen.org/][Leiningen]]: Ferramenta de build para Clojure
- [[https://ollama.com/][Ollama]]: Modelo de linguagem local
...
2. /Sistema de embeddings/
   - Criação de embeddings para o texto usando [[https://pt.wikipedia.org/wiki/TF-IDF][TF-IDF]]
   - Busca por similaridade lexical (bag-of-words)
3. /Interface com o LLM/
   - Geração de resposta usando o LLM

#+BEGIN_QUOTE
/Observação:/ Embora o RAG moderno utilize embeddings densos gerados por modelos de linguagem para capturar a semântica de forma mais rica, neste artigo, usaremos uma implementação simplificada de [[https://pt.wikipedia.org/wiki/TF-IDF][TF-IDF (Term Frequency-Inverse Document Frequency)]] como /prova de conceito/. Para aplicações em produção, recomendamos fortemente o uso de embeddings densos.
#+END_QUOTE


*** TF-IDF

O [[https://pt.wikipedia.org/wiki/TF-IDF][TF-IDF]] (Term Frequency-Inverse Document Frequency) é uma técnica estatística usada para avaliar a importância de uma palavra em um documento, em relação a uma coleção de documentos. Vamos entender como funciona:
...
- Doc1: "Clojure é uma linguagem funcional baseada em [[https://en.wikipedia.org/wiki/Lisp_(programming_language][Lisp]])"
...
#+BEGIN_QUOTE
Para requisitos detalhados de produção e otimizações avançadas, consulte o [[#requisitos-de-hardware-detalhados][apêndice de hardware]] ao final do artigo.
#+END_QUOTE
...
#+BEGIN_QUOTE
/Importante/: Dividimos o texto em chunks usando tokens (não caracteres) para não ultrapassar o limite do modelo. A contagem de tokens é aproximada. Para produção, use uma biblioteca como [[https://github.com/justone/clojure-tiktoken][clojure-tiktoken]] para maior precisão.
#+END_QUOTE
...
#+BEGIN_QUOTE
/Importante/: Esta implementação TF-IDF é uma /prova de conceito/ para demonstrar os fundamentos do RAG. Em aplicações reais, embeddings densos modernos como [[https://www.sbert.net/][SBERT]], [[https://huggingface.co/intfloat/e5-large][E5]], [[https://huggingface.co/BAAI/bge-large-en][BGE]] ou modelos via Ollama superam significativamente o TF-IDF em tarefas de busca semântica e question-answering.
#+END_QUOTE
...
1. /Instalação/:
   - /Windows/: Baixe o instalador do [[https://ollama.com/download][site oficial do Ollama]] e execute-o
...
#+BEGIN_QUOTE
/NOTA:/ A propósito, o projeto docai está disponível no [[https://github.com/lobovit/docai][https://github.com/lobovit/docai]] caso você queira contribuir com o projeto ou usar em outro projeto.
#+END_QUOTE
...
Nossa implementação atual oferece uma base funcional, mas pode ser significativamente otimizada em termos de performance através da adoção de bancos de dados vetoriais como [[https://milvus.io/][Milvus]] ou [[https://github.com/facebookresearch/faiss][FAISS]], implementação de cache de embeddings e paralelização do processamento de chunks, permitindo consultas mais rápidas mesmo com grandes volumes de dados.
...
Quanto à escolha de modelos no ecossistema Ollama, cada um apresenta características distintas que podem ser exploradas conforme a necessidade: o [[https://ollama.com/models/deepseek-r1][DeepSeek R1]] destaca-se na compreensão geral e geração de texto, o [[https://ollama.com/models/deepseek-coder][DeepSeek Coder]] é especializado em código, o [[https://ollama.com/models/llama3][Llama 3]] serve como excelente alternativa geral, o [[https://ollama.com/models/mistral][Mistral]] demonstra eficácia em tarefas específicas, enquanto o [[https://ollama.com/models/gemma][Gemma]] oferece uma solução leve e eficiente para ambientes com recursos limitados.
...
4. /Melhorias Futuras/ - Implementar [[https://en.wikipedia.org/wiki/Exponential_backoff][retry com backoff exponencial]] para falhas de conexão, adicionar [[https://en.wikipedia.org/wiki/Cache_(computing][cache de embeddings]]) para melhor performance, implementar [[https://en.wikipedia.org/wiki/Streaming_media][streaming]] para arquivos muito grandes, adicionar [[https://en.wikipedia.org/wiki/Document_validation][validação de formato de documentos]] e implementar [[https://en.wikipedia.org/wiki/Rate_limiting][rate limiting]] para evitar sobrecarga do Ollama.
...
#+BEGIN_QUOTE
Para técnicas avançadas de prompt engineering, consulte o [[https://www.promptingguide.ai/][Guia Completo de Prompt Engineering]].
#+END_QUOTE
...
Olha, dá pra turbinar esse nosso RAG de várias formas! Primeiro, a gente poderia melhorar a tokenização usando aqueles métodos mais avançados tipo [[https://en.wikipedia.org/wiki/Byte_pair_encoding][BPE]] ou [[https://en.wikipedia.org/wiki/WordPiece][WordPiece]] - idealmente o mesmo que o modelo usa. 
...
Quando o projeto crescer, vai ser essencial ter um banco de dados vetorial decente. Imagina lidar com milhares de documentos usando nossa implementação atual? Seria um pesadelo! [[https://milvus.io/][Milvus]], [[https://github.com/facebookresearch/faiss][FAISS]] ou [[https://qdrant.tech/][Qdrant]] resolveriam isso numa boa. E não podemos esquecer do cache - tanto para embeddings quanto para respostas. Economiza um tempão e reduz a carga no sistema.
...
- [[https://www.pinecone.io/learn/rag/][RAG]] - Documentação do Pinecone
- [[https://www.pinecone.io/learn/embeddings/][Embedding]] - Documentação do Pinecone
- [[https://www.pinecone.io/learn/llms/][LLM]] - Documentação do Pinecone
- [[https://ollama.com/][Ollama]] - Ferramenta para rodar LLMs localmente
- [[https://clojure.org/][Clojure]] - Documentação do Clojure
- [[https://github.com/http-kit/http-kit][http-kit]] - Cliente HTTP para Clojure
- [[https://github.com/clojure/data.json][data.json]] - Biblioteca JSON para Clojure
- [[https://clojure.github.io/clojure/clojure.test-api.html][clojure.test]] - Documentação da biblioteca de testes do Clojure
- [[https://github.com/clj-kondo/clj-kondo][clj-kondo]] - Linter para Clojure
