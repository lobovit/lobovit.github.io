...
O modelo de tratamento estruturado de exceções evoluiu desde os anos 60, ganhando formas mais concretas em linguagens como [[https://en.wikipedia.org/wiki/PL/I][PL/I]], [[https://en.wikipedia.org/wiki/Ada_(programming_language)][Ada]]), [[https://en.wikipedia.org/wiki/C%2B%2B][C++]] e [[https://en.wikipedia.org/wiki/Java_(programming_language)][Java]]), e depois sendo adotado por outras como [[https://en.wikipedia.org/wiki/JavaScript][JavaScript]].
...
A linguagem /[[https://en.wikipedia.org/wiki/PL/I][PL/I]]/ (1964) foi pioneira ao introduzir um sistema estruturado de tratamento de condições excepcionais através do constructo ~ON ... DO~ para casos como falhas de operações de /[[https://en.wikipedia.org/wiki/Input/output][I/O]]/ (entrada/saída) e outros erros de execução.

Esse mecanismo de /[[https://en.wikipedia.org/wiki/Error_handling#Error_handlers][handlers]]/ de erro já demonstrava a vantagem de estruturar o código para lidar separadamente com situações de erro. Em essência, o modelo PL/I possuía:
...
Um exemplo simplificado em pseudo-sintaxe inspirada no [[https://en.wikipedia.org/wiki/PL/I][PL/I]] ilustrava essa estrutura:
...
[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/refs/heads/main/blog/content/post/images/trycatch/01.png]]
...
[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/refs/heads/main/blog/content/post/images/trycatch/02.png]]
...
O diagrama a seguir mostra dois fluxos possíveis de um programa simples: no caminho normal, a função é executada e imprime uma mensagem ("Esta linha...") antes de retornar ao ~main~ e encerrar normalmente; já no caminho de erro, a função lança uma exceção (~std::runtime_error~), que não é capturada em nenhuma parte do programa, resultando na /chamada imediata de ~std::terminate()~/ e encerramento abrupto da aplicação. Note que no diagrama, o caminho vermelho representa o comportamento típico no GCC/Clang: ~std::terminate()~ é chamado, e o unwinding pode ser incompleto (dependendo da implementação).

[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/refs/heads/main/blog/content/post/images/trycatch/03.png]]

Podemos observar, em rosa, o ponto onde "dá ruim" (onde a exceção é lançada) e, em vermelho, o caminho do erro levando à chamada de ~std::terminate()~. Esse comportamento é intencional: como o próprio [[https://en.wikipedia.org/wiki/Bjarne_Stroustrup][Stroustrup]] explica, /"se uma função encontrar um erro que não consiga resolver, ela lança uma exceção; alguma função acima na hierarquia de chamadas pode capturá-la, mas, se ninguém o fizer, o programa termina"/.

/Detalhes do Comportamento no Diagrama/: O caminho vermelho "Exceção não capturada → ~std::terminate()~ chamado" reflete o comportamento padrão do GCC/Clang. O comportamento de unwinding para exceções não capturadas é /definido pela implementação/ (implementation-defined): algumas plataformas executam unwinding completo antes de terminar, outras não. No GCC e Clang, por padrão, ~std::terminate()~ é chamado sem unwinding completo, o que significa que objetos em frames superiores (como variáveis locais em ~main~) podem não ter seus destrutores executados.

/Importante sobre Stack Unwinding/: Quando uma exceção não é capturada, o comportamento do C++ é /definido pela implementação/ (implementation-defined): algumas implementações executam o stack unwinding completo antes de chamar ~std::terminate()~, outras chamam ~std::terminate()~ imediatamente sem unwinding. No GCC e Clang, por padrão, ~std::terminate()~ é chamado sem unwinding completo , destrutores só são executados para objetos nos frames de pilha que foram efetivamente desempilhados. Objetos em frames superiores podem não ter seus destrutores executados, dependendo da implementação.
...
1. /No-throw guarantee/ (sem falha): A operação não pode falhar , garantido por funções marcadas com ~noexcept~
...
Outro problema é que esse uso inadequado de exceções [[https://en.wikipedia.org/wiki/Stack_trace][polui o stack-trace]], tornando mais difícil depurar e analisar o comportamento do sistema. O excesso de exceções desnecessárias pode mascarar erros reais, dificultar o profiling e tornar o código mais difícil de manter.
...
Beleza, mas como decidir, de forma objetiva, o que é “inesperado”? A resposta clássica vem do /[[https://en.wikipedia.org/wiki/Design_by_contract][Design by Contract (DbC)]]/ de [[https://en.wikipedia.org/wiki/Bertrand_Meyer][Bertrand Meyer]].
...
1. [[https://a.co/d/8ZBw0ix][/"The Pragmatic Programmer: Your Journey to Mastery"/ - David Thomas & Andrew Hunt]]
...
2. [[https://a.co/d/1L2Bwz4][/"Effective Modern C++: 42 Specific Ways to Improve Your Use of C++11 and C++14"/ - Scott Meyers]]
   /Discute técnicas modernas de C++, incluindo o uso correto de exceções e alternativas como ~std::optional~./
3. [[https://a.co/d/3Wy2dFE][/"Programming: Principles and Practice Using C++"/ - Bjarne Stroustrup]]
   /O criador do C++ explica fundamentos da linguagem, incluindo tratamento de erros e quando usar exceções./
4. [[https://a.co/d/a4zoUcs][/"The Rust Programming Language" (Livro Oficial)/ - Steve Klabnik & Carol Nichols]]
   /Explica o sistema de ~Result~ e ~Option~ do Rust, que evita exceções./
5. [[https://a.co/d/4geTFbr][/"Clojure for the Brave and True"/ - Daniel Higginbotham]]
   /Aborda a filosofia de tratamento de erros em Clojure usando valores e mapas./
6. [[https://a.co/d/8oEH9z4][/"Designing Data-Intensive Applications"/ - Martin Kleppmann]]
   /Discute tolerância a falhas em sistemas distribuídos, complementando o conceito de "graceful failure"./
7. [[https://a.co/d/66ya4UP][/"Release It!: Design and Deploy Production-Ready Software"/ - Michael T. Nygard]]
    /Ensina padrões como "Circuit Breaker" para lidar com erros em produção./
8. [[https://a.co/d/5bg0IIB][/"Functional Light JavaScript"/ - Kyle Simpson]]
    /Mostra como aplicar conceitos funcionais (incluindo tratamento de erros sem exceções) em JavaScript./
9. [[https://a.co/d/9S37n8W][/"Domain Modeling Made Functional"/ - Scott Wlaschin]]
    /Usa F# para demonstrar como tipos como ~Result~ podem modelar erros de forma explícita./

/Referências Técnicas e Standards:/

10. [[https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines][/C++ Core Guidelines/]]
    /Diretrizes oficiais para C++ moderno, especialmente:/
    - /[[https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#e3-use-exceptions-for-error-handling-only][E.3: Use exceptions for error handling only]]/
    - /[[https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#e12-use-noexcept-when-exiting-a-function-because-of-a-throw-is-impossible-or-unacceptable][E.12: Use noexcept when exiting a function because of a throw is impossible or unacceptable]]/
    - /[[https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#e27-if-you-cant-throw-exceptions-simulate-raii-for-resource-management][E.27: If you can't throw exceptions, simulate RAII for resource management]]/

11. [[https://www.stroustrup.com/abstraction-and-machine.pdf][/"Abstraction and the C++ Machine Model"/ - Bjarne Stroustrup]]
    /Discussão técnica sobre overhead de exceções e otimizações do compilador./

12. [[https://isocpp.org/std/the-standard][/C++23 Standard - Exception Handling/ (ISO/IEC 14882:2024)]]
    /Especificação formal de ~std::terminate~, stack unwinding e ~std::expected~. Seções relevantes: [except] (15), [support.exception] (18.8)./

13. [[https://isocpp.org/wiki/faq/exceptions][/"Exception Handling Considered Harmful"/ vs. /"Exception Handling Considered Useful"/]]
