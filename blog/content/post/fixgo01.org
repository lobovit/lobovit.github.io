...
[[https://go.dev/tour/concurrency/2][Canais]] em Go é uma daquelas features que parecem simples até você precisar usá-la em produção com centenas de [[https://go.dev/tour/concurrency/1][goroutines]] concorrentes. E é aí que "a jiripoca começa a piar".

Este artigo conta a história de um bug silencioso que encontrei durante uma revisão de código no [[https://github.com/lobovit/Ollanta][Ollanta]], um projeto pessoal de análise de código estático multi-linguagem escrito em Go. O bug não quebrava testes, não produzia [[https://go.dev/blog/defer-panic-and-recover][panic]], não gerava erro no [[https://golangci-lint.run][linter]]. Mas, em condições específicas, impedia o [[https://learn.microsoft.com/pt-br/dotnet/core/extensions/graceful-shutdown][graceful shutdown]] do scanner por tempo indeterminado.

![[/post/images/fixgo01.png][ollanta]]

#+BEGIN_QUOTE
/saiba mais:/ /graceful shutdown/ é quando um programa para de aceitar trabalho novo e conclui o que já está em andamento antes de desligar. O contrário seria matar o processo abruptamente, perdendo trabalho pela metade. Em Go, isso geralmente se faz com [[https://pkg.go.dev/context][~context.Context~]] e [[https://pkg.go.dev/os/signal#NotifyContext][~signal.NotifyContext~]].
#+END_QUOTE

* O contexto

O Ollanta precisa analisar centenas de arquivos fonte em paralelo. Para isso, usa um [[https://gobyexample.com/worker-pools][worker pool]], um conjunto fixo de goroutines que processam tarefas de uma fila com semáforo implementado via [[https://go.dev/tour/concurrency/2][channel]]:
...
O número de workers é [[https://pkg.go.dev/runtime#NumCPU][~runtime.NumCPU()~]] ~* 2~. Numa máquina com 8 cores, 16 goroutines processam arquivos simultaneamente. O semáforo controla esse limite. Visualmente, o fluxo é uma fila com gargalo controlado: os 200 arquivos descobertos em disco são empurrados para o semáforo, que só deixa passar 16 de cada vez. Cada worker que pega um arquivo o analisa e devolve o resultado. Os outros 184 ficam esperando na porta.
...
#+BEGIN_QUOTE
/saiba mais:/ [[https://pkg.go.dev/sync#WaitGroup][~sync.WaitGroup~]] é um contador atômico usado para esperar que várias goroutines terminem. Cada ~wg.Add(1)~ incrementa, cada ~defer wg.Done()~ decrementa ao sair da goroutine, e ~wg.Wait()~ bloqueia até o contador zerar. O [[https://go.dev/tour/flowcontrol/12][~defer~]] garante que ~wg.Done()~ seja chamado mesmo se a goroutine retornar por erro ou [[https://go.dev/blog/defer-panic-and-recover][~panic~]]. Já ~recover()~ captura um panic e evita que ele derrube o programa inteiro, aqui ele serve para isolar a falha a um único arquivo sem abortar a análise dos demais.
#+END_QUOTE
...
- /O linter não alerta/: não há regra no [[https://golangci-lint.run][golangci-lint]] que detecte "context check after blocking channel operation" — você está por conta própria.
...
Canais em Go parecem simples, mas o diabo está na ordem. O padrão de semáforo com channel é ensinado em todos os tutoriais de [[https://go.dev/tour/concurrency/1][concorrência em Go]], mas poucos mencionam a interação com [[https://pkg.go.dev/context][contextos canceláveis]]. A maioria dos exemplos assume que a operação vai completar rápido, mas em produção não vai!
...
#+BEGIN_QUOTE
Este artigo foi escrito a partir de uma correção real no repositório do [[https://github.com/lobovit/Ollanta][Ollanta]]. O diff completo está em [[https://github.com/lobovit/Ollanta/commit/3d95d5399b5cb0143637cce51e787ce8bb752285][https://github.com/lobovit/Ollanta/blob/main/application/scan/executor.go]].
#+END_QUOTE
