...
O /[[https://prometheus.io/][Prometheus]]/ é uma ferramenta open-source de monitoramento de sistemas e aplicações que revolucionou a forma de pensar observabilidade em ambientes distribuídos. Ele coleta e armazena métricas como séries temporais, ou seja, valores numéricos associados a um carimbo de tempo e a pares chave-valor chamados /[[https://prometheus.io/docs/concepts/data_model/#metric-names-and-labels][labels]]/. 

#+BEGIN_QUOTE
A potência do Prometheus vem, em parte, da sua linguagem de consulta própria, /[[https://prometheus.io/docs/prometheus/latest/querying/basics/][PromQL]]/, que permite criar consultas complexas para analisar os dados coletados em tempo real. A interface web integrada (Expression browser) facilita visualizar e explorar métricas, possibilitando análises rápidas para identificar tendências e anomalias.
#+END_QUOTE

Desenvolvido inicialmente na SoundCloud em 2012 por [[https://github.com/juliusv][Julius Volz]] e equipe, o Prometheus foi projetado para ser simples, eficiente e altamente dimensionável. Em 2016, o projeto foi adotado pela /[[https://www.cncf.io/][Cloud Native Computing Foundation (CNCF)]]/ como o segundo projeto hospedado (logo após o [[https://kubernetes.io/][Kubernetes]]), reforçando sua maturidade e ampla adoção pela comunidade. 
...
[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/main/post/images/tsdb/prom-pullvspush.png]]
...
A arquitetura do Prometheus foi concebida para facilitar a coleta de dados de múltiplas fontes de forma confiável e distribuída. O coração do sistema é o /[[https://prometheus.io/docs/prometheus/latest/components/prometheus/][Prometheus Server]]/ principal, responsável por agendar e realizar as coletas (/scrapes/) de cada alvo monitorado e armazenar as séries temporais resultantes localmente. 

A configuração dessas coletas é definida em um arquivo YAML (geralmente ~prometheus.yml~), especificando /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#job_name][jobs]]/ e /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#static_configs][targets]]/ – por exemplo, "coletar métricas do serviço X na URL Y a cada 15 segundos". A figura abaixo (extraída da documentação oficial) ilustra a arquitetura e os componentes do ecossistema Prometheus:

[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/refs/heads/main/blog/content/post/images/tsdb/arch.png]]

Em resumo, o fluxo é: o Prometheus /coleta (pull)/ métricas dos jobs instrumentados, diretamente dos serviços ou via um componente intermediário de push para jobs efêmeros. Todos os samples coletados são armazenados localmente no banco de dados de séries temporais embutido ([[https://prometheus.io/docs/prometheus/latest/storage/tsdb/][TSDB]]). 

Regras definidas podem ser executadas continuamente sobre esses dados – seja para gravar novas séries agregadas ([[https://prometheus.io/docs/prometheus/latest/configuration/recording_rules/][recording rules]]) ou para acionar /[[https://prometheus.io/docs/prometheus/latest/configuration/alerting_rules/][alertas]]/. Os alertas gerados pelo Prometheus são então enviados para o /[[https://prometheus.io/docs/alerting/latest/alertmanager/][Alertmanager]]/ processar. Por fim, ferramentas de visualização como o /[[https://grafana.com/][Grafana]]/ podem consultar o Prometheus para exibir dashboards das métricas coletadas.

O ecossistema Prometheus possui diversos componentes (muitos opcionais) que interagem nessa arquitetura:

/ /[[https://prometheus.io/docs/prometheus/latest/components/prometheus/][Servidor Prometheus]]* – o servidor principal que coleta e armazena as métricas e processa consultas PromQL.
/ /[[https://prometheus.io/docs/instrumenting/clientlibs/][Bibliotecas cliente]]* – usadas para instrumentar código de aplicações (expondo métricas via /metrics). Há libs oficiais em Go, Java, Ruby, Python, etc.
/ /[[https://prometheus.io/docs/instrumenting/exporters/][Exporters]]* – programas externos que coletam métricas de serviços ou sistemas terceiros (bancos de dados, servidores web, sistemas operacionais) e as expõem no formato Prometheus. Exemplos: Node Exporter (métricas de sistema Linux), Blackbox Exporter (monitoramento de endpoints externos), etc.
/ /[[https://prometheus.io/docs/instrumenting/pushing/][Pushgateway]]/ – gateway para receber métricas /pushed* por aplicativos de curta duração ou ambientes onde não dá para o Prometheus puxar diretamente.
/ /[[https://prometheus.io/docs/alerting/latest/alertmanager/][Alertmanager]]* – componente responsável por receber alertas enviados pelo Prometheus e gerenciar o envio de notificações (email, Slack, PagerDuty etc.), realizando agrupamento, deduplicação e silenciamento conforme configurado.
/ /[[https://prometheus.io/docs/prometheus/latest/tools/][Ferramentas de suporte]]* – englobam utilitários de linha de comando (como o promtool), exportadores de terceiros, dashboards pré-configurados, entre outros, que facilitam operar e integrar o Prometheus.

Essa arquitetura descentralizada (com coleta /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#pull_interval][pull]]/ e componentes distintos) torna o Prometheus especialmente adequado a ambientes modernos com microsserviços e orquestração de contêineres ([[https://www.docker.com/][Docker]], [[https://kubernetes.io/][Kubernetes]]). 
...
No Prometheus, /[[https://prometheus.io/docs/concepts/data_model/#metric-names-and-labels][labels]]/ (rótulos) e /[[https://prometheus.io/docs/concepts/data_model/#samples-and-series][samples]]/ (amostras) são conceitos-chave para organizar os dados monitorados.
...
Da mesma forma, no Prometheus cada métrica pode ter vários /[[https://prometheus.io/docs/concepts/data_model/#metric-names-and-labels][labels]]/ (chave=valor) que a qualificam. 
...
Os /[[https://prometheus.io/docs/concepts/data_model/#metric-names-and-labels][labels]]/ permitem um modelo de dados multidimensional – ou seja, uma mesma métrica (ex: ~http_requests_total~) é armazenada separadamente para cada combinação de labels (rota="/login", método="GET", código="200", etc.). Isso enriquece as análises, pois podemos agregar ou dividir métricas por essas dimensões conforme necessário.

[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/tsdb/samples01.png]]

Já os /[[https://prometheus.io/docs/concepts/data_model/#samples-and-series][samples]]/ são as unidades de dado coletadas ao longo do tempo – cada medição individual de uma métrica em um determinado instante.
...
Existem diversas maneiras de instalar e executar o Prometheus. Aqui vou demonstrar uma configuração simples usando /[[https://www.docker.com/][Docker]]/ e /[[https://docs.docker.com/compose/][Docker Compose]]/, incluindo o Grafana e uma ferramenta de simulação de métricas chamada /[[https://github.com/dmitsh/promsim][PromSim]]/ (útil para testes). Essa stack de exemplo traz:

/ /[[https://prometheus.io/][Prometheus]]* – servidor de métricas.
/ /[[https://grafana.com/][Grafana]]* – para dashboards e visualização.
/ /[[https://github.com/dmitsh/promsim][PromSim]]* – um simulador que expõe métricas aleatórias para exercitar o Prometheus.
...
[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/tsdb/ui01.png]]
...
/ /[[/classic/targets][Help]]*: link para documentação e ajuda do Prometheus.

Além disso, logo abaixo dos menus, a UI oferece algumas opções e campos importantes:

/ /[[https://prometheus.io/docs/prometheus/latest/querying/basics/#time-range-and-resolution-selection][Time range e refresh]]*: controles para selecionar o intervalo de tempo da consulta e atualizar automaticamente.
/ /[[https://prometheus.io/docs/prometheus/latest/querying/basics/#time-range-and-resolution-selection][Use local time]]*: alterna entre exibir os timestamps no seu fuso horário local ou em UTC.
/ /[[https://prometheus.io/docs/prometheus/latest/querying/basics/#query-history][Query history]]*: opção para habilitar histórico das consultas feitas (facilita repetir queries recentes).
/ /[[https://prometheus.io/docs/prometheus/latest/querying/basics/#autocomplete][Autocomplete]]*: opção para habilitar auto-completar de métricas e funções no campo de consulta.
/ /[[https://prometheus.io/docs/prometheus/latest/querying/basics/#expression-language-promql][Campo de consulta PromQL]]*: onde você escreve a expressão a ser consultada. O Prometheus traz sugestões enquanto você digita (se autocomplete ligado).
/ /[[https://prometheus.io/docs/prometheus/latest/querying/basics/#execute-and-reset][Botões Execute / Reset]]*: para executar a consulta ou limpar o campo.
/ /[[https://prometheus.io/docs/prometheus/latest/querying/basics/#graph-and-table][Aba Graph / Table]]*: seleciona se o resultado será plotado em um gráfico ou mostrado como tabela bruta de valores.
/ /[[https://prometheus.io/docs/prometheus/latest/querying/basics/#evaluation-time][Evaluation time]]*: permite fixar um timestamp específico para avaliar a query (por padrão é "now", mas você pode ver valores históricos escolhendo um horário passado).
...
Após instalar, o principal arquivo a ajustar é o de /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/][configuração do Prometheus]]/ (~prometheus.yml~). Nele definimos os parâmetros globais, jobs de scrape, regras de alerta, etc. Vamos examinar a estrutura básica e algumas customizações comuns. Um exemplo mínimo de ~prometheus.yml~ poderia ser:
...
Para ambientes com muitos alvos ou infraestrutura dinâmica, é inviável gerenciar esses targets manualmente. Nesses casos, o Prometheus oferece integrações de /Service Discovery/ ([[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#kubernetes_sd_config][Kubernetes]], [[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#ec2_sd_config][AWS EC2]], [[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#consul_sd_config][Consul]], [[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#dns_sd_config][DNS]], etc.) e também o /file-based discovery/ (descoberta via arquivos). 
...
Exemplo de definição de retenção no /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#configuration-file][systemd]]/ (ExecStart):
...
Depois reinicie o serviço/container do Prometheus para aplicar as alterações. Para validar se a sintaxe do arquivo está correta antes de reiniciar, podemos usar o /[[https://prometheus.io/docs/prometheus/latest/tools/promtool/][promtool]]/ conforme abaixo.
...
Significa instrumentar o próprio código da aplicação ou serviço para expor métricas de negócio ou de desempenho relevantes. Você adiciona pontos de métrica no código ([[https://prometheus.io/docs/concepts/metric_types/#counter][counters]], [[https://prometheus.io/docs/concepts/metric_types/#gauge][gauges]], etc.) usando uma biblioteca cliente do Prometheus.
...
Refere-se a coletar métricas de sistemas externos ou legados através de componentes intermediários chamados /[[https://prometheus.io/docs/instrumenting/exporters/][exporters]]/. Em vez de modificar o sistema alvo, você roda um exporter que coleta informações daquele sistema (geralmente via APIs existentes, comandos ou leitura de arquivos) e as expõe no formato Prometheus. 
...
Por exemplo, há exporters para /[[https://github.com/prometheus/mysqld_exporter][MySQL]]/, /[[https://github.com/prometheus/postgres_exporter][PostgreSQL]]/, /[[https://github.com/nginxinc/nginx-prometheus-exporter][Apache/Nginx]]/, /[[https://github.com/oliver006/redis_exporter][Redis]]/, entre muitos outros, que traduzem métricas desses sistemas para o formato esperado.
...
/ /[[https://github.com/prometheus/node_exporter][Node Exporter]]* (Linux): Coleta métricas de sistema operacional Linux – CPU, memória, disco, rede, entropia, stats de kernel, etc. É imprescindível para monitorar VMs ou servidores bare metal. Basta executar o binário do node\_exporter no host; ele abre :9100/metrics com dezenas de métricas padronizadas (cpu\_seconds\_total, node\_filesystem\_usage\_bytes, etc.). Essas métricas dão uma visibilidade completa do estado do host, permitindo identificar gargalos de recurso.

/ /[[https://github.com/prometheus/wmic_exporter][Windows Exporter]]* (Windows): Equivalente para plataformas Windows (antigo WMI exporter). Coleta CPU, memória, disco, contadores do Windows, etc., expondo em :9182/metrics (porta padrão). Assim, ambiente heterogêneos também podem ser monitorados.

/ /[[https://github.com/prometheus/blackbox_exporter][Blackbox Exporter]]/: Útil para monitorar /externamente/ a disponibilidade de serviços. Ele executa /probes* do tipo ICMP (ping), HTTP(S), DNS, TCP, etc., simulando a experiência do usuário externo. Você configura módulos de probe (ex: checar HTTP 200 em determinada URL dentro de 2s) e o Prometheus chama o Blackbox passando o alvo a testar. Se a resposta falha ou excede tempo, métricas como ~probe_success~=0 ou ~probe_duration_seconds~ indicam problema. É excelente para monitorar uptime de sites e endpoints de fora para dentro.

/ /[[https://prometheus.io/docs/instrumenting/exporters/][Exporters de aplicações]]/: Há muitos: PostgreSQL exporter, Redis exporter, JMX exporter (Java), SNMP exporter (equipamentos de rede), etc. Em geral, se você usar alguma tecnologia popular, provavelmente já existe um exporter pronto (a documentação oficial lista dezenas: /[[https://prometheus.io/docs/instrumenting/exporters/][Exporters e integrações]]*).
...
/ /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#kubernetes_sd_config][Kubernetes]]*: Descobre pods, serviços, endpoints automaticamente baseado em labels e anotações.
/ /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#ec2_sd_config][AWS EC2]]*: Encontra instâncias EC2 baseado em tags.
/ /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#consul_sd_config][Consul]]*: Usa o Consul como fonte de verdade para serviços.
/ /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#dns_sd_config][DNS]]*: Resolve nomes DNS para descobrir alvos.
/ /[[https://prometheus.io/docs/prometheus/latest/configuration/configuration/#file_sd_config][File-based]]*: Lê targets de arquivos JSON/YAML que podem ser atualizados externamente.
...
Agora vejamos como instrumentar aplicações escritas em algumas linguagens populares. A ideia geral em qualquer linguagem é: instalar a biblioteca cliente do Prometheus, criar métricas ([[https://prometheus.io/docs/concepts/metric_types/#counter][counters]], [[https://prometheus.io/docs/concepts/metric_types/#gauge][gauges]], etc.) em pontos estratégicos do código, e expor um endpoint HTTP ~/metrics~ onde essas métricas são servidas (em formato de texto). O Prometheus então coleta nesse endpoint.

**** Java (Micrometer / Cliente Java do Prometheus)

Em Java, uma abordagem comum é usar o /[[https://micrometer.io/][Micrometer]]/ – uma biblioteca de instrumentação que suporta múltiplos backends (Prometheus, Graphite, etc.). O Micrometer foi adotado pelo Spring Boot, por exemplo, facilitando a exposição de métricas. Passos básicos:
...
/ /[[https://prometheus.io/docs/instrumenting/writing_exporters/#writing-a-bridge-exporter][Bridges personalizadas]]:/ Caso não exista um exporter pronto, podemos criar um processo intermediário (/bridge*) que consulta a ferramenta legada de alguma forma (API REST, CLI, leitura de arquivos de log) e expõe resultados em /metrics. Essencialmente, isso é escrever um pequeno exporter sob medida. Ferramentas de script como Python facilitam isso – você coleta os dados e usa ~prometheus_client~ para expor.

/ /[[https://prometheus.io/docs/instrumenting/writing_exporters/#writing-a-bridge-exporter][Integrações via gateway ou plugins]]:* Alguns ambientes possuem hooks para métricas. Por exemplo, aplicações .NET legadas podem exportar contadores no Windows Performance Counters – aí usar o Windows Exporter para pegá-los. Em casos extremos, você pode usar o Pushgateway como ponte: o sistema legado faz push de alguma métrica básica para o gateway (não ideal, mas possível).
...
4. /Executar e testar:/ Rode o Alertmanager com esse config (no Docker ou binário). Quando um alerta InstanceDown ocorrer, o Prometheus vai enviar para o Alertmanager, que em seguida usará a integração [[https://prometheus.io/docs/alerting/latest/configuration/#slack-receiver][Slack]] para postar no canal configurado uma mensagem com título "Instância X fora do ar" e descrição com detalhes.

Esse foi um exemplo focado em Slack, mas o Alertmanager suporta muitos outros /receivers/: e-mail (SMTP), PagerDuty, OpsGenie, VictorOps, Webhooks genéricos, entre outros. Com ele, você ganha flexibilidade para gerenciar o "barulho" de alertas: por exemplo, suprimir alertas filhos quando um pai já ocorreu ([[https://prometheus.io/docs/alerting/latest/configuration/#inhibition][inhibition]]), ou silenciar certos alertas durante janelas de manutenção planejada.
...
Para outros cenários, onde o push é considerado porque há firewall/NAT impedindo scrapes, a documentação sugere alternativas melhores – como rodar Prometheus perto dos alvos (dentro da rede) ou usar algo como o /[[https://github.com/prometheus/pushprox][PushProx]]/ para atravessar firewalls mantendo o modelo pull. E para jobs cron por máquina, que têm contexto de host, recomenda-se usar o /[[https://github.com/prometheus/node_exporter#textfile-collector][Node Exporter Textfile Collector]]/ (escrever métricas em um arquivo que o Node Exporter lê), ao invés do Pushgateway.
...
Nesta seção, vamos dissecar o funcionamento interno do armazenamento de dados do Prometheus – o /[[https://prometheus.io/docs/introduction/architecture/#time-series-database][Time Series Database]]/ (TSDB) local – e entender por que ele consome recursos como consome.
...
Cada bloco de 2h é identificado por um /[[https://github.com/prometheus/prometheus/blob/main/tsdb/encoding/ulid.go][ULID]]/ (ID único lexicograficamente ordenável) que compõe o nome da pasta. Dentro de um bloco, temos:
...
Além dos blocos fechados, existe o /[[https://prometheus.io/docs/introduction/architecture/#head-block][Head block]]/ (bloco atual em memória) que armazena as métricas em curso. Os dados mais recentes (últimas \~2h) residem em memória para escrita rápida e consultas de curtíssimo prazo.
...
/ /ulid:* Identificador único do bloco (um código 128-bit parecido com [[https://en.wikipedia.org/wiki/Universally_unique_identifier][UUID]]). Ele é também o nome da pasta do bloco.
...
/ /stats:* Estatísticas do bloco – quantas amostras ([[https://prometheus.io/docs/introduction/architecture/#head-block][numSamples]]), séries ([[https://prometheus.io/docs/introduction/architecture/#head-block][numSeries]]) e chunks ([[https://prometheus.io/docs/introduction/architecture/#head-block][numChunks]]) estão armazenados nele. No exemplo real acima, temos \~1,34 milhão de séries distintas, totalizando 553 milhões de amostras em \~4,44 milhões de chunks dentro desse bloco de 2h. Esses números dão uma noção do volume de dados.
/ /compaction:/ Informa o histórico de compactação. /[[https://prometheus.io/docs/introduction/architecture/#head-block][level]]/ indica quantas vezes já foi compactado (1 significa um bloco resultante da junção de outros menores). /[[https://prometheus.io/docs/introduction/architecture/#head-block][sources]]* lista os IDs dos blocos que foram combinados para formar este (no caso, dois blocos anteriores). Se o bloco foi gerado direto do Head (dados “originais”), às vezes sources contém ele próprio.
...
O índice é altamente otimizado e comprimido – usa conceitos de [[https://prometheus.io/docs/introduction/architecture/#posting-lists][posting lists]] (listas de IDs de séries para cada label-valor) e [[https://prometheus.io/docs/introduction/architecture/#symbol-table][tabelas de símbolos]] para strings únicas. Esses detalhes avançados fogem do escopo aqui, mas o importante é: o índice permite que mesmo com milhões de séries por bloco, o Prometheus consiga localizar dados sem varrer tudo linearmente.

Finalmente, o /[[https://prometheus.io/docs/introduction/architecture/#write-ahead-log][WAL (Write-Ahead Log)]]/ é um log de transações recente onde cada amostra coletada é gravada imediatamente no disco antes de ser inserida na memória do Head. Isso garante que, se o Prometheus cair inesperadamente, ao voltar ele pode reprocessar o WAL e recuperar as amostras que ainda não tinham sido compactadas em blocos.
...
[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/main/post/images/tsdb/prom-mem02.png]]
...
[[https://raw.githubusercontent.com/lobovit/lobovit.github.io/main/post/images/tsdb/prom-mem03.png]]
...
/ /Head Block (memória):/ Novas séries e amostras entram aqui. As séries ativas ocupam estruturas na heap da aplicação Go do Prometheus. A cada amostra recebida, ela também é anexada no /[[https://prometheus.io/docs/introduction/architecture/#write-ahead-log][WAL]]* (no SSD/disco) para registro permanente. Durante até \~2h, os dados ficam disponíveis no Head para consultas instantâneas. Por isso, consultas e alertas em dados "frescos" são muito rápidas.

/ /Flush para bloco persistente:/ Quando o intervalo de 2h se completa, o Prometheus corta o bloco (na verdade ele espera 2h ou 1h30 dependendo de certas condições) e escreve um /[[https://prometheus.io/docs/introduction/architecture/#head-block][novo bloco]]* no diretório data (contendo aqueles 2h de amostras agora imutáveis, já comprimidas). Em seguida, libera da memória boa parte das estruturas referentes àquele intervalo. O head então mantém somente as séries ainda ativas que extrapolem o próximo bloco.
...
/ /Documentação Oficial do Prometheus* – especialmente a [[https://prometheus.io/docs/introduction/overview/][Overview]] , [[https://prometheus.io/docs/concepts/metric_types/][Metric Types]] , [[https://prometheus.io/docs/practices/naming/][Best Practices]] e seção de [[https://prometheus.io/docs/prometheus/latest/storage/][Storage]] .
/ /Blog Robust Perception (Brian Brazil)* – várias postagens aprofundadas, por exemplo: [[https://www.robustperception.io/federation-what-is-it-good-for/]["Federation, what is it good for?"]] , [[https://www.robustperception.io/how-much-ram-does-prometheus-2-x-need-for-cardinality-and-ingestion/]["How much RAM does Prometheus 2.x need..."]] , [[https://www.robustperception.io/using-json-file-service-discovery-with-prometheus]["Using JSON file service discovery"]] .
/ /Ganesh Vernekar – Série de artigos "Prometheus TSDB"/ – /Parts 1-7* no blog do Ganesh (engenheiro Grafana Labs) detalhando a fundo a arquitetura do TSDB. Em especial, [[https://ganeshvernekar.com/blog/prometheus-tsdb-persistent-block-and-its-index/][Parte 4: Blocos persistentes e Índice]] .
...
/ /Grafana Mimir* – [[https://grafana.com/oss/mimir/][Página oficial]]  e anúncio do lançamento em 2022, mostrando como escalar Prometheus para 1 bilhão de séries.
