...
Tudo começa com o armazenamento de /1 bit/. Como vimos no estudo de portas lógicas, um /[[https://pt.wikipedia.org/wiki/Flip-flop][flip-flop]]/ tipo D (acionado por nível) é construído com um inversor, duas portas AND e duas portas NOR.
...
Quando você pode alterar os sinais de endereço à vontade para ler ou escrever em qualquer local instantaneamente, você tem uma /[[https://pt.wikipedia.org/wiki/RAM][RAM]]/ (Random Access Memory).
...
A solução mágica da eletrônica para isso é o /[[https://en.wikipedia.org/wiki/Three-state_logic][Buffer Tri-State]]/ (Três Estados).
...
Para alimentar a [[https://pt.wikipedia.org/wiki/Unidade_lógica_e_aritmética][ALU]] rapidamente sem depender do acesso lento à RAM o tempo todo, a CPU possui "bancadas de trabalho" internas super-rápidas chamadas /[[https://pt.wikipedia.org/wiki/Registrador_(inform%C3%A1tica][Registradores]])/.
...
Para coordenar tudo isso, a CPU utiliza /[[https://pt.wikipedia.org/wiki/Barramento_(inform%C3%A1tica][Barramentos]])/ (Buses):
