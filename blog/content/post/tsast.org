...
Vamos usar uma biblioteca popular e robusta pra parsear código JavaScript/TypeScript e gerar uma AST compatível com o padrão [[https://github.com/estree/estree][ESTree]], que é amplamente usado no ecossistema JavaScript (ESLint, Babel, Prettier, etc.). A ~@typescript-eslint/typescript-estree~ é perfeita pra isso, pois usa o próprio compilador do TypeScript por baixo dos panos.

/1. Preparando o Ambiente:/

Primeiro, crie um projetinho [[https://nodejs.org/][Node.js]] básico (se ainda não tiver um) e instale as dependências:
