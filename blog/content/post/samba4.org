...
/ /[[#auditando-acessos][Rotacionamento de logs]]*
/ /[[#auditando-acessos][Acls para permissões avançadas no Linux]]*
...
Tudo começou em 1983, quando a /[[www.ibm.com][IBM]]/ e a Sytec co-desenvolveram um sistema de rede simples projetado para a construção de pequenas redes locais. O sistema era baseado numa API para comunicação em redes chamada /[[https://pt.wikipedia.org/wiki/NetBIOS][NetBIOS]]/, ou /Net/work /B/asic /I/nput /O/utput /S/ystem, isto é, /Sistema Básico de Rede de Entrada/Saída/.

Foi incluso no NetBIOS um esquema de endereçamento que usava nomes de 16 bytes para identificar estações de trabalho e aplicações habilitadas para rede. Em seguida, a /[[www.microsoft.com][Microsoft]]/ adicionou recursos para o DOS que permitia o disco I/O ser redirecionado para a interface NetBIOS, /o que permitiu que o espaço em disco fosse compartilhável através da LAN/. O protocolo de compartilhamento de arquivos ficou conhecido como /[[https://pt.wikipedia.org/wiki/Server_Message_Block][SMB]]/, e agora /[[http://www.webopedia.com/TERM/C/CIFS.html][CIFS]]/.

Em 1985, o protocolo foi expandido, dando origem ao protocolo /[[https://pt.wikipedia.org/wiki/NetBEUI][NetBEUI]]/, que foi durante muito tempo o principal protocolo usado em redes locais, antes da popularização do TCP/IP. O /[[https://pt.wikipedia.org/wiki/Server_Message_Block][SMB]]/ (Server Message Block) veio mais tarde, junto com o Windows 3.11. O protocolo SMB faz o compartilhamento de arquivos e impressoras em redes Microsoft, incluindo a navegação na rede, o estabelecimento de conexões e a transferência de dados. Ele utiliza o NetBIOS para a troca de mensagens entre os hosts e inclui uma versão atualizada do protocolo, que roda sobre o TCP/IP.

/[[https://pt.wikipedia.org/wiki/Andrew_Tridgell][Andrew Tridgell]]/, um australiano que na época era estudante do curso de PhD em Ciências da Computação da /[[www.anu.edu.au][Universidade Nacional da Austrália]]/, precisava rodar um software chamado /[[http://www.hpl.hp.com/hpjournal/dtj/vol4num1/vol4num1art7.pdf]["eXcursion"]]/, no /[[https://en.wikipedia.org/wiki/Digital_Equipment_Corporation][DEC]]/, empresa que trabalhava, para interagir com o /[[https://en.wikipedia.org/wiki/Pathworks][Patchworks]]/, um software de compartilhamento de arquivos. O /[[https://en.wikipedia.org/wiki/Pathworks][Patchworks]]/ era um software proprietário, que utilizava um protocolo obscuro, sobre o qual não existiam muitas informações disponíveis.

![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/andrew.jpg#center][Samba]]

Como todo bom hacker Andrew_tridgell decidiu estudar o protocolo e assim desenvolver um servidor que pudesse rodar em seu PC. Ele desenvolveu então um /[[https://pt.wikipedia.org/wiki/Sniffing][sniffer]]/ chamado /clockspy/ que era capaz de examinar o tráfego da rede e fez engenharia reversa no protocolo SMB e o implementou no Unix. Isso fez com que no servidor Unix aparecesse como um servidor de arquivos Windows em seu PC com DOS. Com isso, ele foi rapidamente capaz de implementar o suporte às principais chamadas e a desenvolver um programa servidor que era capaz de conversar com os clientes rodando o Patchworks. O objetivo desta primeira versão era apenas resolver um problema doméstico: interligar um computador rodando o MS-DOS ao servidor rodando o /[[https://pt.wikipedia.org/wiki/Solaris][Solaris]]/.
...
No entanto, em 1999 a Microsoft lançou o Active Directory que é implementado pela primeira vez no /[[https://pt.wikipedia.org/wiki/Windows_2000][Microsoft Windows 2000 Server Edition]]/ e com melhorias significativas no Microsoft Windows Server 2003 e 2003 R2 e mais tarde no Windows Server 2008 e 2008 R2.

#+BEGIN_QUOTE
Com isto, surgiu o desafio da comunidade que desenvolvia o Samba na época (e que desenvolve até hoje), a criar uma implementação de serviço de diretório usando um protocolo compatível com o /[[https://pt.wikipedia.org/wiki/LDAP][LDA]]/ que armazenasse informações sobre objetos na rede e ao mesmo tempo disponibilizasse essas informações a usuários e administradores da mesma rede.
#+END_QUOTE

O problema é que não havia interesse por parte da Microsoft em compartilhar o /[[https://pt.wikipedia.org/wiki/Request_for_Comments][RFC]]/ (Request for Comments), isto é, o documento que descreve os padrões de cada protocolo de Internet antes de serem considerados de fato um padrão como fora feito com o protocolo LDAP por exemplo. Com isto, a comunidade que desenvolve o Samba não fazia ideia de como o AD funcionava. Basicamente, teriam que pesquisar anos a fio e colocar mais sniffers em rede, fazer engenharia reversa e mais uma vez demorar anos e anos em desenvolvimento para criar uma solução alternativa ao /[[https://pt.wikipedia.org/wiki/Active_Directory][AD]]/.

Usando destes recursos então a comunidade começa a escrever uma nova versão pós a 3.6 do Samba em 2011. A /[[https://www.samba.org/samba/history/samba-3.6.25.html][versão 3.6]]/ já trabalhava com o protocolo LDAP usando o /[[https://pt.wikipedia.org/wiki/OpenLDAP][OpenLDAP]]/ e o /[[https://pt.wikipedia.org/wiki/Kerberos][Kerberos]]/ em separado para alimentar as funcionalidades do Samba para que pudessem obter funcionalidades próximas ao AD.
...
#+BEGIN_QUOTE
O nome "Samba" surgiu a partir de uma simples busca dentro do dicionário /[[https://en.wikipedia.org/wiki/Ispell][Ispell]]/ por palavras que possuíssem as letras S, M e B, de "Server Message Blocks", posicionadas nessa ordem. A busca retornou apenas as palavras "salmonberry", "samba", "sawtimber" e "scramble", de forma que a escolha do nome acabou sendo óbvia.
#+END_QUOTE
...
A primeira versão a usar o nome Samba, foi a 1.6.05, que foi a sucessora imediata do "smbserver 1.6.4". O projeto anteriormente fora apresentado por /[[https://pt.wikipedia.org/wiki/Andrew_Tridgell][Andrew Tridgell]]/ com o nome de /[[http://ftp.samba.org/ftp/samba/old-versions/server-05.tar.Z][nbserver 0.5]]/ que foi o início do projeto que veio a tornar-se Samba mais tarde. Ao todo se considerar-mos os patches, o Samba contém mais de 50 versões até chegar a versão atual. Sem os patches e\ou versões no estilo /[[https://pt.wikipedia.org/wiki/CVS][CVS]]/, o Samba tem exatamente 4 versões desde a 1.x.x até a recente 4.x.x que podem ser vistos no /[[https://www.samba.org/samba/history/][histórico de versões do samba]]/. O registro da primeira versão do que seria o samba hoje, foi anunciado por Andrew na [[https://en.wikipedia.org/wiki/Usenet][Usenet]] (hoje arquivada em no Google Groups) e ainda pode ser visto /[[https://groups.google.com/forum/#!msg/vmsnet.networks.desktop.pathworks/EmyZEGd5ZRA/nJtBfp6ak30J][aqui]]/.
...
/ /1.9.16/ Foi quando foi adotado o /[[https://pt.wikipedia.org/wiki/CVS][CVS]]* o que permitiu que o Samba fosse desenvolvido mais rapidamente.
/ /2.0/ O Samba se tornou ainda mais popular e acessível aos usuários finais. Neste mesmo ano 1999, fora lançado o /[[https://www.samba.org/samba/docs/using_samba/ch00.html][primeiro livro sobre o Samba]]* que hoje faz parte da documentação oficial do mesmo.
/ /2.2/ Esta se torna a primeira versão estável do Samba. Nesta versão, já é possível criar um domínio primário para /[[https://en.wikipedia.org/wiki/Windows_NT_4.0][NT4]]/, suporta Windows 2000 e é melhorada a parte de infra-Estrutura para impressão /(graças ao apoio da HP)* e comunidade.
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/gpo.gif#center][Samba]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/sysvol.gif#center][Samba]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/smb1.png#center][SMB]]

Basicamente o cliente se conecta ao servidor que utiliza o protocolo TCP/IP (na verdade, se conecta ao NetBIOS sobre o TCP/IP, conforme especificado no /[[http://tools.ietf.org/html/rfc1001][RFC1001]]/ e /[[http://tools.ietf.org/html/rfc1002][RFC1002]]/), NetBEUI ou IPX/SPX. Depois de terem estabelecido uma conexão, o cliente pode, em seguida, enviar comandos (SMBs) para o servidor o que lhe permite abrir arquivos, ler e escrever. É possível fazer todo o tipo de coisa dentro de um servidor de arquivos. No entanto, no caso do SMB, estas coisas são feitas através da rede.
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/ad1.png#center][SMB]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/ad2.png#center][SMB]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/ad3.png#center][SMB]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/ad4.png#center][SMB]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/ad5.png#center][SMB]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/ad6.png#center][SMB]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/ad7.png#center][SMB]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/ad8.png#center][SMB]]
...
O projeto OpenLDAP foi iniciado em 1998 por /[[http://www.openldap.org/project/kurt/][Kurt Zeilenga]]/. O projeto começou como um fork do projeto LDAP a partir da Universidade de Michigan.
...
#+BEGIN_QUOTE
/Nota:/ /[[https://pt.wikipedia.org/wiki/Network_Time_Protocol][O Network Time Protocol (NTP)]]/ é um protocolo de rede para sincronização do relógio entre sistemas e computadores além de  medir a latência da rede basicamente.
#+END_QUOTE
...
#+BEGIN_QUOTE
/Nota:/ É de suma importância que a senha do --adminpass seja segura (com números,letras e caracteres especiais). Caso contrário, retornará um erro e não irá promover o samba. Dei preferência ao SAMBA_INTERNAL como sistema de configuração do DNS. Se você prefere usar o BIND9, dê uma estudada em /[[https://wiki.samba.org/index.php/Configure_BIND_as_backend_for_Samba_AD][Configure BIND as backend for SAMBA AD]]/.
#+END_QUOTE
...
Existem dois tipos de ACLs: regras de acesso /(access ACLs)/ e regras padrão /(default ACLs)/. Uma regra de acesso especifica informações de acesso para um único arquivo ou diretório. Já uma regra padrão é aplicada apenas a diretórios, e especifica informações de acesso padrões para todos os arquivos no diretório que não possuam uma ACL explícita aplicada. Como vimos no capítulo 4 em /[[http://lobovit.github.io/2015/09/11/samba4.html#instalando-o-samba][Instalando o Samba]]/, note que instalamos também as ACL's e ajustamos o particionamento para aceitar a acl no /etc/fstab. De agora em diante vamos trabalhar com uma maneira diferente de lidar com permissões além do já conhecido chmod.
...
Observe o sinal de "+" em -rw-rwxr--+. Este sinal indica que o arquivo possui uma lista de acesso /(acl)/ definida. Se emitirmos o mesmo comando para um diretório teremos um resultado diferente. Para explorar mais das acl's, sugiro que dêem uma lida neste artigo escrito por /[[http://www.hardware.com.br/dicas/acl-linux.html][Luis Felipe Silveira]]/.
...
/ /--dns-backend/: Por padrão o Samba é configurado com o servidor DNS interno. Mas você poderá usar o BIND9 como explica no capítulo 4 em /[[ihttp://lobovit.github.io/2015/09/11/samba4.html#samba-como-um-controlador-de-dominio][Samba como um controlador de dominio]]*.
...
Por fim, no samba4 faça alguns testes do kerberos e do próprio samba4 que já foram descritos os passos no capítulo 4 em /[[http://lobovit.github.io/2015/09/11/samba4.html#samba-como-um-controlador-de-dominio][Samba como um controlador de dominio]]/, a partir de /Teste o Kerberos.../.

#+BEGIN_QUOTE
/Nota:/ Caso você tenha problemas com SID duplicados, por favor acessar a documentação do Samba em /[[https://goo.gl/m1lyQJ][Prevent failure due to duplicate SID's]]/. Isto é, lá contém um script em python que ajuda a resolver este problema.
#+END_QUOTE
...
Mais sobre o Samba-Tool na /[[https://www.samba.org/samba/docs/man/manpages/samba-tool.8.html][documentação oficial]]/.
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad1.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad2.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad3.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad4.png#center][Windows]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad5.png#center][Windows]]
...
Para administrar o samba pode-se utilizar o samba-tool, no entanto, recomenda-se a ferramenta /[[http://www.microsoft.com/downloads/details.aspx?FamilyID=7D2F6AD7-656B-4313-A005-4E344E43997D&displaylang=en][Remote Server Admininstration Tools - RSAT da Microsoft]]/. Após instalar, habilite as ferramentas necessárias para a administração do Samba, vá em painel de controle, "Programas e Recursos" e cliquem em "Ativar ou desativar recursos do Windows" (aba lateral esquerda)e selecione as ferramentas:
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad6.png#center][Windows]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad7.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad8.png#center][Windows]]

Para outras versões do Microsoft Windows, consulte a /[[https://wiki.samba.org/index.php/Installing_RSAT_on_Windows_for_AD_Management][instalação do RSAT na documentação do samba]]/. Ao instalar e configurar o Samba, fora definido um compartilhamento denominado "home", que destina-se ao uso pessoal dos usuários. Não se trata da função de perfil remoto, mas apenas um diretório onde o usuário poderá utilizar para armazenar arquivos pessoais e acessar em qualquer computador do domínio.
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad9.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad10.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad11.png#center][Windows]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad12.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad13.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad14.png#center][Windows]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad15.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad16.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad17.png#center][Windows]]

Após criá-lo, consulte-o em "Usuários" e configure o compatilhamento "home", conforme:

![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad18.png#center][Windows]]
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad19.png#center][Windows]]

Ao concluir, basta efetuar o login com o novo usuário criado:

![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad20.png#center][Windows]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad23.png#center][Windows]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad24.png#center][Windows]]
...
#+BEGIN_QUOTE
/Nota:/ Certifique-se de que o usuário estará logando em máquinas onde o Sistema Operacional se mantém o mesmo. Isto é, que ele esteja logando em máquinas do Windows 7 ao invés de outras versões. Pois, pode ser que não funcione caso o usuário fique transitando entre uma versão e outra do mesmo sistema. Caso ainda tenha dificuldades em operar com perfil nomade no windows7, dê uma olhada /[[http://samba.org/tridge/Samba4Demo/s4demo3.ogv][neste vídeo]]/. Recomendo que tenha o VLC player instalado na máquina.
#+END_QUOTE
...
Para trabalharmos com o unidades organizacionais (UO) no samba, poderemos fazer isto usando o /[[https://msdn.microsoft.com/en-us/library/bb742437.aspx#EEAA][snap-in]]/ do Windows, ou até mesmo o Zentyal, GOsa2, Webmin, Smb4k, o Lam, SMB2WWW, gnomba, Jabs, Komba2, Smb Web client e muitos outros. No entanto, como a maioria dos administradores de sistemas estão mais acostumados a trabalhar com o /[[https://msdn.microsoft.com/en-us/library/bb742437.aspx#EEAA][snap-in]]/ do Windows, a este darei preferência por enquanto. Mais a frente farei uma abordagem ao GOsa2 e outras tecnologias.
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad25.png#center][Windows]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad26.png#center][Windows]]
* Clique sobre com botão direito e  Criar um GPO neste domínio e …
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad27.png#center][Windows]]
* Nome da GPO será “Exemplo_GPO” e OK.
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad28.png#center][Windows]]
* Sobre a nova GPO clique com botão direito,  clique em  editar:
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad29.png#center][Windows]]
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad30.png#center][Windows]]

Duplo clique sobre a diretiva e marque a opção  “Habilitado” de OK. Pronto, GPO criada com sucesso. Caso ainda existam dúvidas a respeito da criação de GPO's, dê uma olhada neste /[[http://samba.org/tridge/Samba4Demo/s4demo2.ogv][video da documentação oficial]]/.
...
![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad31.png#center][Windows]]

Aqui você deve ser capaz de ver os seus controladores de domínio disponíveis e seus status como mostrado abaixo:

![[https://raw.githubusercontent.com/lobovit/lobovit.github.io/master/post/images/samba/win7ad32.png#center][Windows]]

Agora teste criar um usuário no servidor secundário e acompanhar no primário como já mostrei no capítulo 6 em /[[http://lobovit.github.io/2015/09/11/samba4.html#samba-tool][Samba tool]]/. Se o usuário criado no servidor secundário aparecer no primário, a nossa replicação está funcionando bem. O mesmo a partir de qualquer controlador de domínio.

-----

*** Referências:

/ /[[https://www.packtpub.com/web-development/implementing-samba-4][Implementing Samba 4]]*
/ /[[http://www.amazon.com/Active-Directory-Cookbook-Cookbooks-OReilly/dp/1449361420][Active Directory Cookbook]]*
/ /[[https://openlibrary.org/books/OL12369507M/The_Definitive_Guide_to_Samba_4][The Definitive Guide to Samba 4]]*
