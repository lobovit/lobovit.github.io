...
Como quando você abre uma IDE e tem à sua disposição ferramentas que parecem mágicas como [[https://en.wikipedia.org/wiki/Intelligent_code_completion][intelisense]], [[https://en.wikipedia.org/wiki/Autocompletion][autocompletion]], [[https://en.wikipedia.org/wiki/Variable_dumping][dumping de variáveis]] e [[https://en.wikipedia.org/wiki/Reserved_word][palavras reservadas]] de cada linguagem de programação, entre outras. Neste artigo, vou compartilhar um pouco do que eu aprendi e ainda estou aprendendo sobre análise estática de código com o intuito de ajudar você a entender um pouco mais sobre como esses mecanismos funcionam.
...
Claro que essa "mágica" tem seus truques e limitações. Às vezes ela pode te encher de avisos sobre problemas que nem existem, ou deixar passar alguns bugs bem na sua cara. Mas mesmo assim, é uma ferramenta poderosa que pode salvar seu dia, seu código e projeto como um todo. A análise estática sempre fica nesse dilema entre [[https://en.wikipedia.org/wiki/Soundness][(soundness)]] e [[https://en.wikipedia.org/wiki/Precision_(statistics][(precision)]]). 
...
- [[#reticulados-os-organizadores-da-bagunça][Reticulados: os organizadores da bagunça]]
  - [[#exemplo-reticulado-de-tipos-ou-a-família-dos-tipos][Exemplo: Reticulado de Tipos]]
- [[#análise-de-tipos-o-sherlock-holmes-do-seu-código][Análise de Tipos]]
- [[#análise-de-ponteiros-e-alias-quem-está-apontando-pra-quem][Análise de Ponteiros e Alias]]
  - [[#os-três-detetives-do-caso][Diferentes Abordagens de Investigação]]
  - [[#o-super-poder-da-sensibilidade-ao-fluxo][Sensibilidade ao Fluxo]]
  - [[#analisando-o-perigo-dos-nulls-o-detetive-anti-crash][Análise de Null Pointers]]
  - [[#quando-funções-entram-na-jogada][Análise Interprocedural]]
- [[#interpretação-abstrata-a-arte-de-simplificar-sem-perder-a-essência][Interpretação Abstrata]]
  - [[#análise-de-intervalos-descobrindo-os-valores-secretos-das-variáveis][Análise de Intervalos]]
...
- [[#representações-de-programa][Representações de Programa]]
  - [[#árvore-sintática-abstrata-ast-o-esqueleto-do-código][Árvore Sintática Abstrata (AST)]]
  - [[#grafo-de-fluxo-de-controle-cfg-o-mapa-da-execução][Grafo de Fluxo de Controle (CFG)]]
- [[#técnicas-avançadas][Técnicas Avançadas]]
  - [[#sensibilidade-de-contexto-entendendo-o-de-onde-veio][Sensibilidade de Contexto]]
  - [[#algoritmos-de-ponto-fixo-deixa-comigo-que-eu-resolvo-rápido][Algoritmos de Ponto Fixo]]
  - [[#imutabilidade-a-arma-secreta-contra-bugs-cabeludos][Imutabilidade]]
  - [[#modelos-de-atores-e-análise-de-mensagens-cada-um-no-seu-quadrado-e-se-falando][Modelos de Atores e Análise de Mensagens]]
- [[#otimização-e-desempenho][Otimização e Desempenho]]
  - [[#casos-especiais-e-otimizações-a-vida-como-ela-é-e-como-deixar-ela-mais-rápida][Casos Especiais e Otimizações]]
  - [[#como-os-mestres-fazem-compiladores-reais][Como os "Mestres" Fazem]]
  - [[#desempenho-de-analisadores-estáticos-por-que-uns-são-tão-rápidos-e-outros-tão][Desempenho de Analisadores Estáticos]]
...
A análise estática tem uma história e tanto que começou lá no finalzinho dos anos 1950, quando o [[https://en.wikipedia.org/wiki/John_Backus][John Backus e sua turma criaram o primeiro compilador FORTRAN]]. Backus, junto com o [[https://en.wikipedia.org/wiki/Peter_Naur][Peter Naur]], bolou um jeito de formalizar a sintaxe das linguagens de programação com a tal da [[https://en.wikipedia.org/wiki/Backus%E2%80%93Naur_form][BNF (Backus-Naur Form)]], que virou peça-chave pra análise sintática.

[[https://i.pinimg.com/736x/ca/94/9f/ca949f962ceff9ce15036525a5d1e5be--programming-languages-ibm.jpg]]

Na área de verificação de programas, o [[https://en.wikipedia.org/wiki/Robert_W._Floyd][Robert W. Floyd]] soltou em 1967 um trabalho muito bom chamado [[https://en.wikipedia.org/wiki/Assigning_Meanings_to_Programs]["Assigning Meanings to Programs"]], que abriu caminho pro que a gente conhece hoje como [[https://en.wikipedia.org/wiki/Hoare_logic][lógica de Hoare]]. Floyd teve a sacada de usar invariantes de loop e asserções pra provar que os programas faziam o que deviam. O Naur também meteu o bedelho nessa área, batendo na tecla de que especificações claras e métodos na régua eram essenciais pra validar programas.

Nos anos 1970, o [[https://en.wikipedia.org/wiki/Stephen_Cook][Stephen Cook]] e outros caras começaram a dar uma cara mais formal pros fundamentos teóricos da computabilidade ligados à análise estática. Aí em 1977 veio a virada de jogo: [[https://en.wikipedia.org/wiki/Patrick_Cousot][Patrick Cousot e Radhia Cousot]] apresentaram a teoria da [[https://en.wikipedia.org/wiki/Abstract_interpretation][interpretação abstrata]], que virou um dos pilares da análise estática moderna. 
...
Na parte de otimização de código, os compiladores usam análise estática pra entender a fundo o comportamento do programa e gerar um código mais enxuto. Eles identificam [[https://en.wikipedia.org/wiki/Dead_code_elimination][código morto que pode ser jogado fora]], sacam [[https://en.wikipedia.org/wiki/Loop_invariant_code_motion][expressões dentro de loops que sempre dão no mesmo]] (aí dá pra calcular só uma vez fora do loop), descobrem se valores de variáveis dependem do que o usuário digita (se não dependerem, dá pra calcular na hora de compilar), e estimam os valores mínimos e máximos que as variáveis podem ter (ajudando a escolher tipos de dados que economizam memória).

Além disso, os compiladores analisam se funções podem mexer nas mesmas estruturas de dados ao mesmo tempo sem dar rolo, o que permite [[https://en.wikipedia.org/wiki/Parallel_computing][paralelização]]. Um exemplo clássico de otimização é quando juntamos a movimentação de código invariante com transformação de loop. Dá uma olhada nessa função que calcula áreas de círculos:
...
Depois da otimização (aplicando a técnica de [[https://en.wikipedia.org/wiki/Loop_invariant_code_motion][remoção de código invariante de loop]]), o código fica assim:
...
Os tipos de problemas detectados variam de acordo com a linguagem de programação utilizada e a natureza específica da sua aplicação. Em linguagens como C e C++, onde você precisa gerenciar memória na mão, a análise estática ajuda a encontrar [[https://en.wikipedia.org/wiki/Memory_leak][vazamentos de memória]] (quando você esquece de liberar memória), [[https://en.wikipedia.org/wiki/Use-after-free][uso-após-liberação]] (quando você tenta usar memória que já foi liberada), e [[https://en.wikipedia.org/wiki/Null_pointer][ponteiros nulos]] (quando você tenta acessar o endereço zero).  

** Reticulados: Os Organizadores da Bagunça!

Imagine que você está tentando arrumar seu quarto cheio de coisas espalhadas. Um [[https://en.wikipedia.org/wiki/Lattice_(order][/reticulado/]]) é como aquela estante perfeita onde tudo tem seu lugar e você sempre consegue encontrar o que precisa. No mundo da matemática, um reticulado é uma estrutura que organiza elementos em uma ordem parcial, com duas operações especiais:
...
![[./images/type-lattice.png][Reticulado de tipos]]

Esse é um equívoco comum! Na verdade, você provavelmente já usa reticulados sem perceber. Quando trabalha com sistemas de tipos em linguagens como [[https://www.typescriptlang.org/][TypeScript]], [[https://www.rust-lang.org/][Rust]] ou [[https://www.haskell.org/][Haskell]], está navegando por um reticulado de tipos. 

Quando usa análise estática em seu IDE que detecta bugs potenciais antes mesmo de executar o código, há reticulados trabalhando nos bastidores. Até mesmo quando usa um [[https://en.wikipedia.org/wiki/Lint_(software][linter que sugere melhorias no seu código]]), ele está aplicando conceitos baseados em reticulados para analisar o fluxo de dados. Esses conceitos matemáticos, embora pareçam abstratos, são a fundação invisível que torna possíveis muitas das ferramentas que facilitam nosso trabalho diário como desenvolvedores.
...
#+BEGIN_QUOTE
Apesar da brincadeira, [[https://en.wikipedia.org/wiki/Andersen%27s_algorithm][Andersen se refere a um algoritmo de análise de ponteiros]].
#+END_QUOTE
...
#+BEGIN_QUOTE
Assim como Andersen, [[https://en.wikipedia.org/wiki/Steensgaard%27s_algorithm][Steensgaard se refere a um algoritmo de análise de ponteiros]].
#+END_QUOTE
...
Já tomou aquele susto quando o app fecha do nada com [[https://en.wikipedia.org/wiki/NullPointerException][NullPointerException]], hein? Javeiro? É como pisar num buraco que você não viu! Exemplo:
...
O analisador de null pointers é como um cão-guia que late antes de você pisar no buraco! Ele identifica todos os pontos onde você PODE estar tentando usar algo que não existe. No caso do Java, caso tenha o [[https://checkstyle.sourceforge.io/][checkstyle configurado]], ele vai te avisar que você está usando um método de um objeto que pode ser null.
...
A Interpretação Abstrata, com seus [[https://en.wikipedia.org/wiki/Lattice_(order_theory][reticulados]]), [[https://en.wikipedia.org/wiki/Widening_(abstract_interpretation][/widening/]]) e [[https://en.wikipedia.org/wiki/Narrowing_(abstract_interpretation][/narrowing/]]), e [[https://en.wikipedia.org/wiki/Domain_(mathematics][domínios abstratos]]), é uma técnica poderosa para analisar programas. Ela permite que ferramentas encontrem erros e otimizem o código /antes/ de ele ser executado, tornando a programação bem mais interessante!

-----

** Análise de Concorrência

Até agora, focamos em [[https://pt.wikipedia.org/wiki/Programa_sequencial][programas /sequenciais/]], onde as instruções são executadas uma após a outra, em ordem. Mas, no mundo moderno, muitos programas são [[https://pt.wikipedia.org/wiki/Concorrência_(informática][/concorrentes/]]), eles executam várias tarefas "ao mesmo tempo" (ou, pelo menos, dando a /impressão/ de que estão fazendo isso). Isso traz novos desafios para a análise estática. Imagine que você tem duas tarefas:
...
/   /Condições de Corrida [[https://pt.wikipedia.org/wiki/Condição_de_corrida][(Race Conditions)]]:/ Quando o resultado do programa depende da ordem /exata* em que as tarefas concorrentes são executadas (como no exemplo do depósito).
/   [[https://pt.wikipedia.org/wiki/Deadlock][/Deadlocks*]]: Quando duas ou mais tarefas ficam "travadas", esperando umas pelas outras para liberar recursos (tipo, cada uma esperando que a outra termine de usar o banheiro 🚽).
/   [[https://pt.wikipedia.org/wiki/Starvation_(informática][/Starvation*]]): Quando uma tarefa nunca consegue os recursos de que precisa para executar (tipo, ficar sempre em último na fila).
...
O gráfico acima ilustra um problema clássico de concorrência: uma [[https://pt.wikipedia.org/wiki/Condição_de_corrida][race condition]] durante uma operação de depósito bancário. Ele mostra como a Tarefa 1 (depositar R$100) tem duas etapas: primeiro o dinheiro sai da conta de origem e depois entra na conta de destino. 

Se a Tarefa 2 (consultar saldo) for executada exatamente entre essas duas etapas, o usuário verá um saldo incorreto. O gráfico também destaca outros problemas comuns em programas concorrentes: [[https://pt.wikipedia.org/wiki/Condição_de_corrida][race conditions]], [[https://pt.wikipedia.org/wiki/Deadlock][deadlocks]] e [[https://pt.wikipedia.org/wiki/Starvation_(informática][starvation]]).
...
/   /Análise de Fluxo de Dados [[https://en.wikipedia.org/wiki/Dataflow_analysis][(Dataflow Analysis)]]:/  Adapta as técnicas que vimos para programas sequenciais (como [[https://en.wikipedia.org/wiki/Live_variable_analysis][Live Variables]], [[https://en.wikipedia.org/wiki/Reaching_definition][Reaching Definitions]]) para o mundo concorrente.  É preciso levar em conta as possíveis /interferências* entre as tarefas.

/   /Análise Baseada em Tipos [[https://en.wikipedia.org/wiki/Type_system][(Type-Based Analysis)]]:* Usa sistemas de tipos especiais para garantir que certas operações (como o acesso a variáveis compartilhadas) sejam feitas de forma segura.

/   [[https://en.wikipedia.org/wiki/Model_checking][/Model Checking:/]] Constrói um /modelo/ do programa concorrente e explora /todos* os seus possíveis estados e transições, procurando por erros.  É uma técnica poderosa, mas pode ser computacionalmente cara.

/   /Análise Baseada em Grafos [[https://en.wikipedia.org/wiki/Graph_theory][(Graph-Based Analysis)]]:* Representa o programa como um grafo e usa algoritmos de análise de grafos para detectar padrões que indicam problemas de concorrência (como ciclos, que podem indicar deadlocks).

/   /Análise de bloqueios [[https://en.wikipedia.org/wiki/Lock_(computer_science][(lock analysis)]]):/ Verificando se os bloqueios (/locks*) são adquiridos e liberados corretamente.
...
![[./images/andersen-steensgaard.png][Andersen e Steensgaard]]

As análises de [[https://en.wikipedia.org/wiki/Pointer_analysis][Andersen e Steensgaard]] são como dois detetives investigando para onde as variáveis do tipo "ponteiro" (que guardam endereços de memória) estão apontando. A diferença crucial entre eles é como eles lidam com a /informação/ sobre esses apontamentos:

/   /[[https://en.wikipedia.org/wiki/Andersen_analysis][Andersen]] (O Detetive Detalhista):/ Mantém um registro /separado/ para cada variável, anotando /todos/ os lugares para onde ela /pode* estar apontando. É como ter uma ficha completa para cada suspeito.
/   /[[https://en.wikipedia.org/wiki/Steensgaard_analysis][Steensgaard]] (O Detetive Eficiente):/ É mais "econômico" com a informação. Se duas variáveis podem apontar para os /mesmos/ lugares, ele /junta* os registros delas em um só. É como ter uma única ficha para todos os suspeitos que têm o mesmo álibi.
...
1.  /[[https://en.wikipedia.org/wiki/Context-insensitive_analysis][Insensível ao Contexto (O Detetive Distraído):]]/ Não se importa /de onde/ a função foi chamada. Ele trata /todas/ as chamadas da mesma função como se fossem iguais. É como analisar um crime sem considerar o local onde ele ocorreu.
2.  /[[https://en.wikipedia.org/wiki/Context-sensitive_analysis][Sensível ao Contexto (O Detetive Atento):]]/ Leva em conta o /contexto/ de cada chamada da função. Ele analisa a função de forma /diferente/ dependendo de onde ela foi chamada. É como investigar um crime considerando se ele aconteceu em um beco escuro ou em uma praça movimentada.
...
/   /[[https://en.wikipedia.org/wiki/Call-site_sensitivity][Call-site Sensitivity (Sensibilidade ao Local da Chamada):]]/  O detetive olha para o /local* do código onde a função foi chamada (o "call site"). Cada local de chamada é tratado como um caso diferente. É como ter um "carimbo" com o número da linha do código em cada análise.
/   /[[https://en.wikipedia.org/wiki/Object-sensitivity][Object Sensitivity (Sensibilidade ao Objeto):]]/  Muito útil em linguagens orientadas a objetos (como Java). O detetive olha para o /objeto* em que a função foi chamada (o "receptor"). Chamadas em objetos diferentes são tratadas como casos diferentes.
/   /[[https://en.wikipedia.org/wiki/Functional_approach][Functional Approach (Enfoque Funcional):]]* Diferencia com base nos estados abstratos.
...
Uma análise com [[https://en.wikipedia.org/wiki/Object-sensitivity][object sensitivity]] diferenciaria as duas chamadas ~setValue~, uma para ~obj1~ e a outra para ~obj2~. Isso significa:
...
Antes de falar de otimizações, vamos relembrar o que é [[https://en.wikipedia.org/wiki/Static_single_assignment_form][SSA (Static Single Assignment)]] e as [[https://en.wikipedia.org/wiki/Phi_function][funções Phi]]. Na forma SSA, cada variável é atribuída exatamente uma vez, o que facilita várias análises. Quando temos caminhos diferentes no código (tipo um if/else), usamos funções ~Phi (φ)~ para "juntar" os valores:
...
3.  /Variáveis Globais e o "Misterioso" Heap:/ Lembra que o [[https://en.wikipedia.org/wiki/Heap_(data_structure][/heap/]]) é aquela área de memória onde a gente cria objetos dinamicamente? Então, é /difícil/ pra caramba rastrear o que acontece com variáveis globais e coisas no heap. É tipo tentar seguir um gato em um labirinto.

Se a gente tem um código /gigante/, a análise não pode demorar uma eternidade. Então, podemos recorrer a alguns truques:

1.  [[https://en.wikipedia.org/wiki/Control-flow_graph#Topological_sorting][/"Ordem dos Tratores" (Ordenação de Blocos):/]] A gente analisa os pedaços do código (os "blocos") em uma ordem /esperta/. Tipo, se a gente tá indo pra frente, começa do começo. Se tá indo pra trás, começa do fim (dã!). Parece óbvio, mas faz /muita/ diferença.
2.  [[https://en.wikipedia.org/wiki/Incremental_computation][/"Só o que Importa" (Trabalho Incremental):/]] Se só um /pedacinho/ do código mudou, a gente não precisa analisar /tudo/ de novo. É tipo refazer só o pedaço da receita que você errou, não o bolo inteiro.
3.  [[https://en.wikipedia.org/wiki/Widening_and_narrowing][/"Aceleração Inteligente" (Widening e Narrowing):/]] Às vezes, a análise fica "patinando" em um loop. O [[https://en.wikipedia.org/wiki/Widening_and_narrowing][/widening/]] é tipo dar um "salto calculado" pra um valor maior (tipo, "ah, essa variável vai pro infinito!"). Depois, o [[https://en.wikipedia.org/wiki/Widening_and_narrowing][/narrowing/]] tenta "refinar" essa estimativa, pra ficar mais preciso.

*** Como os "Mestres" Fazem (Compiladores Reais)

Os compiladores que a gente usa todo dia (tipo o [[https://en.wikipedia.org/wiki/GNU_Compiler_Collection][GCC]], o [[https://en.wikipedia.org/wiki/LLVM][LLVM]] do [[https://en.wikipedia.org/wiki/Clang][Clang]], e a [[https://en.wikipedia.org/wiki/Java_virtual_machine][JVM]] do [[https://en.wikipedia.org/wiki/Java_(programming_language][Java]])) são /cheios/ de truques:

1.  [[https://en.wikipedia.org/wiki/Intermediate_representation][/"Código Secreto" (Representações Intermediárias Otimizadas):/]] Eles usam umas representações /internas/ do código que são mais fáceis de analisar. É tipo traduzir o código pra uma "língua secreta" que só eles entendem.
2.  [[https://en.wikipedia.org/wiki/Analysis_framework][/"Lego de Análise" (Framework de Análises Plugáveis):/]] Eles têm um monte de pecinhas de análise que a gente pode /combinar/ pra fazer coisas diferentes. É tipo um Lego de análise de código.
3.  [[https://en.wikipedia.org/wiki/Parallel_computing][/"Dividir pra Conquistar" (Paralelização):/]] Eles fazem várias análises ao /mesmo tempo/, em paralelo. É tipo ter vários detetives trabalhando no mesmo caso.
4.  [[https://en.wikipedia.org/wiki/Precision_and_recall][/"Nem Tanto ao Mar, Nem Tanto à Terra" (Equilíbrio Precisão-Desempenho):/]] Eles deixam a gente escolher se quer uma análise /super/ detalhada (mas lenta) ou uma mais rápida (mas que pode deixar passar alguma coisa). E, geralmente, eles só fazem a análise /super/ detalhada nas partes mais /importantes/ do código.

Olha só esse código C++ abaixo que /parece/ simples, mas que dá um trabalhinho pra [[https://en.wikipedia.org/wiki/Live_variable_analysis][análise de variáveis vivas]]:
...
[[https://en.wikipedia.org/wiki/Fixed-point_iteration][Algoritmos de ponto fixo]] são aqueles que ficam iterando repetidamente sobre os dados até que nada mais mude - ou seja, até encontrar um "ponto fixo". Eles são super importantes na análise estática, mas podem demorar muito tempo para convergir. Então, ninguém quer ficar esperando /pra sempre/. Por isso, a galera que manja dos paranauê inventou uns jeitos de acelerar o processo:

1.  [[https://en.wikipedia.org/wiki/Control-flow_graph#Topological_sorting][/"Organizando a Bagunça" (Ordenação de Blocos):/]] Imagina que você tem um monte de caixas pra organizar. Você pode começar por qualquer uma, ou pode ser esperto e organizar primeiro as caixas que vão te ajudar a organizar as outras depois. É tipo isso: a gente analisa os pedaços do código (os "blocos") numa ordem que faz sentido, pra informação "fluir" mais rápido.

2.  [[https://en.wikipedia.org/wiki/Incremental_computation][/"Fazendo Só o Necessário" (Trabalho Incremental):/]] Se você só mudou um pedacinho do código, não precisa analisar /tudo/ de novo. É como consertar um furo no pneu da bicicleta, em vez de trocar a bicicleta inteira. A gente guarda o que já fez antes e só atualiza o que /realmente/ mudou.

3.  [[https://en.wikipedia.org/wiki/Widening_and_narrowing][/"O Chute Controlado" (Widening e Narrowing):/]] Já falamos disso antes, mas vale repetir. É tipo quando você tá perdido e chuta uma direção, mas depois vai ajustando o caminho pra chegar no lugar certo.
...
Os compiladores que a gente usa (tipo o [[https://en.wikipedia.org/wiki/LLVM][LLVM]], o [[https://en.wikipedia.org/wiki/GNU_Compiler_Collection][GCC]], a [[https://en.wikipedia.org/wiki/Java_virtual_machine][JVM]]) são tipo carros de Fórmula 1: /cheios/ de tecnologias pra fazer as coisas /muito/ rápido.  Alguns dos "segredos" deles:

1.  [[https://en.wikipedia.org/wiki/Intermediate_representation][/"Código Ninja" (Representações Intermediárias Otimizadas):/]] Eles transformam o seu código em uma "linguagem secreta" que é mais fácil de analisar. Tipo, o LLVM usa uma coisa chamada "LLVM IR", que é toda organizadinha.  A JVM tem o bytecode, que também é mais "esperto" que o código Java original.

2.  [[https://en.wikipedia.org/wiki/Analysis_framework][/"Caixa de Ferramentas" (Framework de Análises Plugáveis):/]] Eles têm um monte de análises /prontas/ que a gente pode usar, tipo peças de Lego.  E a gente pode até /combinar/ elas pra fazer coisas mais complexas.

3.  [[https://en.wikipedia.org/wiki/Parallel_computing][/"Multitarefa" (Paralelização):/]] Eles fazem /várias/ análises ao /mesmo tempo/, pra economizar tempo. É tipo ter vários cozinheiros preparando o jantar, cada um cuidando de uma parte. E, de novo, se você só mudou um pedacinho do código, eles só analisam aquele pedaço (e o que depende dele).

4.  [[https://en.wikipedia.org/wiki/Precision_and_recall][/"Escolha o Seu Nível" (Equilíbrio Precisão-Desempenho):/]] Você pode escolher se quer uma análise /super/ detalhada (mas que demora) ou uma mais rápida (mas que pode deixar passar alguma coisa).  É tipo escolher o nível de dificuldade em um jogo. E, claro, eles tentam ser /mais/ detalhistas nas partes /mais importantes/ do código. 
...
![[https://i.imgur.com/56aQaCN.png][Imutabilidade]]
...
Lembra que a gente falou de [[https://en.wikipedia.org/wiki/Widening_and_narrowing_conversions][/widening/ e /narrowing/]]? É tipo um "superpoder" pra análise não ficar presa em loops infinitos.
...
- O [[https://en.wikipedia.org/wiki/Rice%27s_theorem][/teorema de Rice/]] nos lembra que verificar propriedades semânticas não-triviais de programas é geralmente indecidível
...
1. [[https://books.google.com.br/books/about/Principles_of_Program_Analysis.html?id=RLjt0xSj8DcC&redir_esc=y][Nielson, F., Nielson, H. R., & Hankin, C. (2004). /Principles of Program Analysis/. Springer.]] - Este livro é um dos mais importantes para entender a análise estática de código.
2. [[https://www.amazon.com/Compilers-Principles-Techniques-Tools-2nd/dp/0321486811][Aho, A. V., Lam, M. S., Sethi, R., & Ullman, J. D. (2006). /Compilers: Principles, Techniques, and Tools/ (2nd ed.). Addison-Wesley.]] - Apesar de ser um pouco antigo, este livro ainda é uma referência essencial para entender como os compiladores funcionam. Inclusive, é referência para a disciplina deCOMPILADORES do curso de Ciência da Computação de muitas universidades.
3. [[https://www.amazon.com/Types-Programming-Languages-MIT-Press/dp/0262162091][ Benjamin C. Pierce . Types and Programming Languages*. MIT Press.]] - Este livro é fundamental para entender a teoria de tipos e a programação funcional.
4. [[https://dl.acm.org/doi/10.1145/357172.357173][Cousot, P., & Cousot, R. (1977). /Abstract Interpretation: A Unified Lattice Model for Static Analysis of Programs by Construction or Approximation of Fixpoints/. ACM SIGACT-SIGPLAN.]] - Este artigo é um dos mais importantes para entender a análise estática de código.
5. [[https://mitpress.mit.edu/books/formal-semantics-programming-languages][Winskel, G. (1993). /The Formal Semantics of Programming Languages: An Introduction/. MIT Press.]] - Este livro é fundamental para entender a semântica formal de linguagens de programação.
6. [[https://mitpress.mit.edu/books/model-checking][Clarke, E. M., Grumberg, O., & Peled, D. A. (1999). /Model Checking/. MIT Press.]] - Este livro é fundamental para entender a modelagem e a verificação de sistemas.
7. [[https://dl.acm.org/doi/10.1145/568570.568571][Ball, T., & Rajamani, S. K. (2002). /The SLAM Project: Debugging System Software via Static Analysis/. ACM SIGPLAN.]] - Este artigo é um dos mais importantes para entender a análise estática de código.
8. [[https://dl.acm.org/doi/10.1145/134554.134560][Landi, W. (1992). /Undecidability of Static Analysis/. ACM Letters on Programming Languages and Systems.]] - Este artigo é um dos mais importantes para entender a análise estática de código.
9. [[https://www.cs.cmu.edu/~crary/819-f09/Andersen94.pdf][Andersen, L. O. (1994). /Program Analysis and Specialization for the C Programming Language/. PhD Thesis, University of Copenhagen.]] - Este artigo é um dos mais importantes para entender a análise estática de código.
10. [[https://dl.acm.org/doi/10.1145/237221.237222][Steensgaard, B. (1996). /Points-to Analysis in Almost Linear Time/. ACM SIGPLAN-SIGACT.]] - Este artigo é um dos mais importantes para entender a análise estática de código.
11. [[https://www.amazon.com/Advanced-Compiler-Design-Implementation-Morgan/dp/1558603204][Muchnick, S. S. (1997). /Advanced Compiler Design and Implementation/. Morgan Kaufmann.]] - Este livro é fundamental para entender a análise estática de código.
12. [[https://dl.acm.org/doi/10.1145/201037.201049][Reps, T., Horwitz, S., & Sagiv, M. (1995). /Precise Interprocedural Dataflow Analysis via Graph Reachability/. ACM SIGPLAN-SIGACT.]] - Este artigo é um dos mais importantes para entender a análise estática de código.
13. [[https://dl.acm.org/doi/10.1145/568570.568571][Flanagan, C., Leino, K. R. M., Lillibridge, M., Nelson, G., Saxe, J. B., & Stata, R. (2002). /Extended Static Checking for Java/. ACM SIGPLAN.]] - Este artigo é um dos mais importantes para entender a análise estática de código.
