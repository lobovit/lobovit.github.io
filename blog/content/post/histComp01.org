...
Ao concluir a leitura do livro [[https://a.co/d/0a3DsSsn][Code: The Hidden Language of Computer Hardware and Software]] 2ª ed. de Charles Petzold, decidi criar uma série de artigos sintetizando seu conteúdo e em Português. Então vamos ao ponto de partida:

O Código Morse foi inventado por volta de 1837 por [[https://pt.wikipedia.org/wiki/Samuel_Morse][Samuel Finley Breese Morse]] (1791–1872). Ele foi aprimorado por outros inventores, notavelmente [[https://en.wikipedia.org/wiki/Alfred_Vail][Alfred Vail]] (1807–1859), e evoluiu para algumas versões diferentes. O sistema que utilizamos e estudamos hoje é mais formalmente conhecido como /[[https://pt.wikipedia.org/wiki/Código_Morse][Código Morse Internacional]]/.

Mais de 180 anos depois, o Código Morse continua tão relevante que, em maio de 2026, um hacker o utilizou para enganar um agente de inteligência artificial. A técnica foi a [[https://pt.wikipedia.org/wiki/Injeção_de_prompt][injeção de prompt]]: comandos maliciosos em código Morse foram enviados ao sistema, que os interpretou como instruções legítimas e transferiu US$ 213 mil (R$ 1 milhão) em criptomoedas ([[https://livecoins.com.br/hacker-usa-codigo-morse-para-fazer-agente-autonomo-de-ia-lhe-enviar-r-1-milhao-em-criptomoeda/][Livecoins]]). O caso mostra que a estrutura binária de pontos e traços que você aprenderá a seguir não é apenas história, ela ainda povoa o submundo da segurança digital.

![[/post/images/hist/grok-morse.png][grok-morse]]

A invenção do código Morse caminha lado a lado com a invenção do [[https://pt.wikipedia.org/wiki/Telégrafo][telégrafo]], o primeiro sistema a transmitir informação codificada por impulsos elétricos através de fios. Essa união entre /código binário/ (ponto/traço) e /sinal elétrico/ (pulso curto/pulso longo) é exatamente a base de como os computadores funcionam, a diferença é que o telégrafo era operado manualmente, enquanto o computador faz tudo automaticamente e bilhões de vezes mais rápido.
...
![[/post/images/hist/morse-map.png][morse-map]]
...
Esta foi a primeira aplicação prática de /[[https://pt.wikipedia.org/wiki/Codificação_por_entropia][codificação por entropia]]/, atribuir símbolos mais curtos a eventos mais frequentes. O mesmo princípio, formalizado matematicamente por [[https://pt.wikipedia.org/wiki/David_Huffman][David Huffman]] em 1952, é o coração dos algoritmos de compressão modernos:

/ /[[https://pt.wikipedia.org/wiki/Codificação_de_Huffman][Codificação de Huffman]]* → ZIP, JPEG, PNG
/ /[[https://pt.wikipedia.org/wiki/Codificação_aritmética][Codificação aritmética]]* → MP4, H.264
/ /[[https://pt.wikipedia.org/wiki/LZ77][Lempel-Ziv (LZ77)]]* → GIF, gzip, PDF
...
[[https://pt.wikipedia.org/wiki/Louis_Braille][Louis Braille]] (1809–1852) perdeu a visão aos três anos de idade em um acidente na oficina de arreios de seu pai: uma /sovela/ (ferramenta perfurante de couro) escorregou e atingiu seu olho. A infecção se espalhou para o outro olho, deixando-o completamente cego.

Aos dez anos, ganhou uma bolsa para o Instituto Real para Jovens Cegos em Paris. Lá, o ensino era baseado no sistema de [[https://pt.wikipedia.org/wiki/Valentin_Haüy][Valentin Haüy]], que consistia em imprimir letras do alfabeto comum em alto-relevo no papel, para que fossem sentidas pelo tato. O problema? Apalpar as curvas complexas das letras tradicionais era extremamente lento, impreciso e frustrante, e os livros eram caríssimos de produzir.

Aos doze anos, Braille conheceu o sistema /écriture nocturne/ (escrita noturna), inventado pelo capitão do exército francês [[https://en.wikipedia.org/wiki/Charles_Barbier][Charles Barbier]] para que soldados pudessem ler mensagens no escuro. Era um código de 12 pontos em relevo representando sons, não letras. Braille enxergou o potencial, mas simplificou o sistema radicalmente: reduziu a matriz para /6 pontos/ (2 colunas, 3 linhas) e o adaptou para representar letras reais.
...
Antes de Morse, Braille ou circuitos elétricos, a civilização chinesa já explorava combinações binárias no /[[https://pt.wikipedia.org/wiki/I_Ching][I Ching]]/ (易经), /O Livro das Mutações/.
...
A coincidência numérica com as 64 células do Braille não é mágica, é matemática. Seis posições binárias sempre geram 64 combinações, independentemente de serem pontos em relevo ou linhas num livro de oráculo. Quando o matemático [[https://pt.wikipedia.org/wiki/Gottfried_Wilhelm_Leibniz][Gottfried Wilhelm Leibniz]] tomou contato com o I Ching no século XVII, reconheceu imediatamente a correspondência com seu sistema binário recém-desenvolvido e viu nisso a confirmação de que a lógica de dois estados não era arbitrária, mas fundamental.
...
Em 1870, o engenheiro francês [[https://pt.wikipedia.org/wiki/Émile_Baudot][Émile Baudot]] resolveu o problema radicalmente: em vez de códigos de tamanho variável, criou um código de /tamanho fixo/, exatos /5 bits/ por caractere, capaz de representar 2⁵ = 32 caracteres. O operador usava um teclado de 5 teclas (como um piano simplificado), e a máquina convertia cada combinação de teclas em 5 sinais elétricos sequenciais. Pela primeira vez na história, uma /máquina/, não um humano, podia codificar e decodificar texto automaticamente.

O código Baudot deu origem aos /[[https://pt.wikipedia.org/wiki/Teleimpressor][teletipos]]/ (teleprinters), que dominaram as comunicações empresariais até meados do século XX, e às /[[https://pt.wikipedia.org/wiki/Fita_perfurada][fitas perfuradas]]/, que foram o principal meio de armazenamento dos primeiros computadores. O termo "[[https://pt.wikipedia.org/wiki/Baud][baud]]" (medida de taxa de transmissão em telecomunicações, como "9600 baud") deriva diretamente do nome de Baudot.
...
Morse e Braille são códigos binários, mas não são códigos /de computador/. A ponte entre esses mundos é o /[[https://pt.wikipedia.org/wiki/ASCII][ASCII]]/ (American Standard Code for Information Interchange), publicado em 1963.
...
Assim como nossos computadores evoluíram, o Braille também precisou crescer. Códigos de turno e escape são geniais, mas adicionam complexidade: você não pode ler um caractere isolado sem saber o que veio antes dele. Para resolver isso em áreas complexas (como notação musical, ideogramas japoneses [[https://pt.wikipedia.org/wiki/Kanji][Kanji]] ou programação de computadores), foi criado o /Braille de 8 pontos/ (adicionando mais uma linha à matriz). A matemática ataca novamente:
...
E se o número 256 soa familiar para você, não é coincidência: /256 é exatamente o número de valores que cabem em 1 [[https://pt.wikipedia.org/wiki/Byte][Byte]] (8 bits) da memória do seu computador ou smartphone atual./ A revolução binária começou muito antes das telas brilharem. O que a princípio eram apenas abstrações matemáticas, códigos de dois estados, combinações, potências de dois, logo encontraria seu lar em fios, interruptores e circuitos elétricos.
