...
Sabe quando você tem uma ideia e quer que o computador a transforme em um aplicativo, um jogo ou um site? A gente usa [[https://www.linguagensdeprogramacao.com.br/][/linguagens de programação/]] pra isso. Elas são como a nossa forma de conversar com a máquina, dando instruções detalhadas para resolver problemas ou criar coisas novas.
...
Pense no compilador como um tradutor superinteligente. Ele pega o código que a gente escreve (que é bem mais fácil de entender) e o traduz para a linguagem que o computador entende. Essa tradução pode ser direta para a linguagem da máquina ou para um formato intermediário, como o [[https://en.wikipedia.org/wiki/Bytecode][bytecode]] ou [[https://webassembly.org/][WebAssembly]], que pode rodar em diferentes lugares, seja no seu PC, no celular ou até no seu navegador.

É por causa dos compiladores que linguagens como [[https://www.rust-lang.org/][Rust]], [[https://go.dev/][Go]] e [[https://www.typescriptlang.org/][TypeScript]] conseguem criar programas super-rápidos, seguros e que funcionam em qualquer plataforma. Eles são a mágica por trás do desempenho de quase tudo que a gente usa no mundo digital.

Hoje em dia, saber como um compilador funciona não é só coisa de professor de faculdade. É o tipo de conhecimento que te dá superpoderes para criar suas próprias linguagens, otimizar programas para rodarem mais rápido em diferentes computadores, ou até para entender como ferramentas como o [[https://v8.dev/][V8]] (o motor do Google Chrome) ou a [[https://www.oracle.com/java/technologies/javase/jvms.html][JVM]] (da linguagem Java) funcionam por dentro. É um campo que junta várias áreas, de lógica a engenharia, e que é essencial para o futuro da [[https://www.inteligenciaartificial.com.br/][Inteligência Artificial]], [[https://www.ciberseguranca.com.br/][cibersegurança]] e [[https://www.games.com.br/][desenvolvimento de games]].
...
Durante todo esse processo de análise, o compilador anota tudo em uma [[https://en.wikipedia.org/wiki/Symbol_table][/tabela de símbolos/]]. Pense nela como um "caderninho de anotações" onde ele guarda informações sobre cada variável e função: o nome, o tipo de dado (se é um número, texto, etc.), e onde ela pode ser usada. Ferramentas modernas, como o [[https://clang.llvm.org/][/Clang/]] e o [[https://www.rust-lang.org/][/Rustc/]], usam essa tabela para dar mensagens de erro super detalhadas e úteis.

Depois que o front-end "entendeu" tudo, o back-end entra em ação. Ele pega a representação intermediária do seu código (como a árvore sintática) e começa a traduzi-la para a linguagem final. Essa linguagem pode ser o código de máquina que a [[https://en.wikipedia.org/wiki/Central_processing_unit][CPU entende]], ou algo como o [[https://en.wikipedia.org/wiki/WebAssembly][/WebAssembly/]] para rodar em múltiplas plataformas.

-----

**** 🌐 /WebAssembly: Evolução de "Navegador" para "Universal"/

O /[[https://en.wikipedia.org/wiki/WebAssembly][WebAssembly (WASM)]]/ surgiu em 2017 como uma tecnologia para rodar código compilado diretamente no navegador, trazendo performance próxima ao nativo para aplicações web. Desde então, evoluiu rapidamente: em 2019, o [[https://wasi.dev/][WASI (WebAssembly System Interface)]] permitiu que módulos [[https://en.wikipedia.org/wiki/WebAssembly][WASM]] acessassem recursos do sistema de forma segura, e em 2022 o [[https://github.com/WebAssembly/component-model][Component Model]] foi padronizado, facilitando a composição de módulos e a criação de plugins e serviços modulares. Hoje, WASM já é alvo de backend para várias linguagens no lado servidor, e a portabilidade é um dos seus maiores trunfos — o mesmo código pode rodar em navegadores, servidores, dispositivos de borda [[https://en.wikipedia.org/wiki/Edge_computing][(edge)]] e [[https://en.wikipedia.org/wiki/Internet_of_things][IoT]].

Essa versatilidade abriu espaço para aplicações em diferentes áreas. No universo serverless e edge computing, plataformas como [[https://developers.cloudflare.com/workers/][Cloudflare Workers]], [[https://docs.fastly.com/products/compute-at-the-edge][Fastly Compute]] e [[https://vercel.com/docs/concepts/functions/edge-functions][Vercel Edge Functions]] executam código WASM globalmente, com baixa latência e alta eficiência, sendo usados em APIs, processamento de dados e autenticação. No entretenimento, engines como [[https://docs.unity3d.com/Manual/webgl-building.html][Unity WebGL]] e [[https://docs.godotengine.org/en/stable/getting_started/workflow/export/exporting_for_web.html][Godot]] exportam jogos completos em WASM, permitindo que rodem em qualquer plataforma sem plugins. No campo da inteligência artificial, frameworks como [[https://www.tensorflow.org/js][TensorFlow.js]] e [[https://onnxruntime.ai/docs/execution-providers/web.html][ONNX Runtime Web]] possibilitam rodar modelos de machine learning diretamente no navegador, com privacidade e aceleração via SIMD e threads.

Além disso, WASM se tornou o backend universal de linguagens modernas: [[https://www.rust-lang.org/][Rust]], [[https://go.dev/][Go]], [[https://en.wikipedia.org/wiki/C_(programming_language][C/C++]]), [[https://www.python.org/][Python]] (via [[https://pyodide.org/][Pyodide]]), [[https://dotnet.microsoft.com/en-us/][C#/.NET]] (via [[https://dotnet.microsoft.com/en-us/apps/aspnet/web-apps/blazor][Blazor]]), [[https://kotlinlang.org/][Kotlin]], [[https://www.assemblyscript.org/][AssemblyScript]] (TypeScript para WASM) e [[https://ziglang.org/][Zig]] já oferecem suporte nativo ou oficial. As linguagens adotam WASM porque ele garante portabilidade real, performance próxima ao nativo, segurança por sandboxing, eficiência no tamanho dos binários e um ecossistema onde o mesmo código pode ser executado em qualquer lugar, do navegador ao servidor, passando por dispositivos embarcados.
...
O [[https://en.wikipedia.org/wiki/WebAssembly][WebAssembly (WASM)]] é revolucionário hoje porque oferece performance próxima ao nativo (10-20x mais rápido que JavaScript), segurança por meio de [[https://en.wikipedia.org/wiki/Sandboxing][sandbox isolado sem acesso direto ao sistema]], portabilidade real com o conceito de "write once, run anywhere", eficiência graças ao tamanho reduzido dos binários e carregamento rápido, além de contar com suporte das principais linguagens de programação. 

Em apenas oito anos, evoluiu de uma tecnologia restrita ao navegador (em 2017) para uma plataforma universal (em 2025), tornando-se alvo de backend para [[https://www.rust-lang.org/][Rust]], [[https://go.dev/][Go]], [[https://www.python.org/][Python]], [[https://dotnet.microsoft.com/en-us/][C#/.NET]], [[https://kotlinlang.org/][Kotlin]], [[https://ziglang.org/][Zig]] e outras linguagens, que agora compilam nativamente para WASM, não apenas para JavaScript. 

#+BEGIN_QUOTE
"Nesse contexto, a otimização realizada pelo back-end do compilador é fundamental: é nessa etapa que o código é ajustado para ser mais rápido, consumir menos energia (algo crucial em dispositivos móveis e [[https://en.wikipedia.org/wiki/Internet_of_things][IoT]]) e tirar proveito de recursos específicos de cada hardware. Ferramentas como o [[https://llvm.org/][LLVM]] desempenham um papel central nesse processo, permitindo que um mesmo back-end produza programas otimizados para uma grande variedade de plataformas, de computadores pessoais a smartphones."
#+END_QUOTE

-----

Agora que entendemos como o [[https://en.wikipedia.org/wiki/WebAssembly][WebAssembly]] e as técnicas modernas de compilação transformaram o cenário da computação, vale a pena olhar para trás e ver como essa evolução aconteceu ao longo das décadas. A seguir, uma linha do tempo destaca os principais marcos da história dos compiladores — do assembly dos mainframes aos frameworks universais e à era da inteligência artificial.
...
Ao longo das décadas, os compiladores passaram por transformações marcantes: nos anos 1960, eram ferramentas acadêmicas voltadas para assembly direto em mainframes; nos anos 1970, surgiram as linguagens de alto nível, trazendo portabilidade e otimizações básicas; os anos 1980 introduziram otimizações avançadas, frameworks e a [[https://en.wikipedia.org/wiki/Cross-compilation][cross-compilation]]; nos anos 1990, destacaram-se a orientação a objetos, a análise de tipos e a compilação [[https://en.wikipedia.org/wiki/Just-in-time_compilation][JIT]]; os anos 2000 trouxeram frameworks modulares como o [[https://llvm.org/][LLVM]] e suporte a múltiplos targets.

A década de 2010 foi marcada pela heterogeneidade, com suporte a [[https://en.wikipedia.org/wiki/Graphics_processing_unit][GPUs]], [[https://en.wikipedia.org/wiki/SIMD][SIMD]] e [[https://en.wikipedia.org/wiki/Parallel_computing][paralelismo]]; nos anos 2020, destacam-se otimizações guiadas por [[https://en.wikipedia.org/wiki/Artificial_intelligence][IA]], [[https://en.wikipedia.org/wiki/WebAssembly][WebAssembly]] e [[https://mlir.llvm.org/][MLIR]]; e, [[https://en.wikipedia.org/wiki/Artificial_intelligence][em 2025, vislumbra-se uma plataforma universal, com suporte a mais de 100 linguagens e aceleradores de IA]]. O resultado desse percurso é a evolução dos compiladores de ferramentas acadêmicas para uma tecnologia fundamental da computação moderna.
...
É assim que o compilador começa a "enxergar" seu código, um pequeno passo de cada vez. E em linguagens como [[https://www.rust-lang.org/][Rust]] ou [[https://www.typescriptlang.org/][TypeScript]], essa etapa já ajuda a verificar se o código é seguro ou se os tipos estão corretos.
...
Os compiladores modernos evoluíram muito além das otimizações tradicionais, incorporando técnicas avançadas como inteligência artificial e o uso de perfis de execução reais para tomar decisões mais inteligentes. Uma dessas técnicas é a [[https://en.wikipedia.org/wiki/Profile-guided_optimization][Profile-Guided Optimization (PGO)]] de segunda geração, que inclui ferramentas como o [[https://github.com/google/autofdo][AutoFDO]], capaz de coletar perfis automaticamente durante a execução normal do programa, e o [[https://github.com/facebook/BOLT][BOLT]], que otimiza o layout do código binário com base em perfis de cache e branch prediction. O resultado dessas abordagens são ganhos de performance reais de 5 a 15%, indo além dos simples benchmarks sintéticos.

Outra inovação importante é o [[https://en.wikipedia.org/wiki/Machine_learning_guided_inlining][Machine-Learning-Guided Inlining (MLGO)]], que utiliza aprendizado de máquina para decidir automaticamente quais funções devem ser expandidas inline. Esses modelos são treinados com milhões de exemplos de código real, permitindo ao compilador reduzir o tempo de compilação em 7 a 15% sem sacrificar a performance do código gerado.

Além disso, a [[https://en.wikipedia.org/wiki/Link-time_optimization][Link-Time Optimization (LTO)]] tornou-se padrão em builds otimizados (~-O2~) nos toolchains modernos como [[https://gcc.gnu.org/][GCC 10+]] e [[https://clang.llvm.org/][Clang 12+]]. O LTO permite que o compilador analise e otimize todo o programa durante o processo de linking, e não apenas arquivos individuais, viabilizando otimizações inter-procedurais que seriam impossíveis ao compilar cada arquivo separadamente.
...
As otimizações modernas de compiladores representam a chamada terceira geração, marcada pelo uso intensivo de dados reais de execução e inteligência artificial. Técnicas como o [[https://en.wikipedia.org/wiki/Profile-guided_optimization][Profile-Guided Optimization (PGO)]] utilizam informações coletadas durante a execução real do programa, em vez de depender apenas de estimativas, permitindo que o compilador tome decisões mais precisas para melhorar a performance. 

O [[https://en.wikipedia.org/wiki/Machine_learning_guided_inlining][Machine-Learning-Guided Inlining (MLGO)]] aplica modelos de aprendizado de máquina treinados com grandes volumes de código do mundo real, identificando padrões e aprendendo quais funções devem ser expandidas inline para otimizar o desempenho. Já a [[https://en.wikipedia.org/wiki/Link-time_optimization][Link-Time Optimization (LTO)]] possibilita uma visão holística do programa, analisando e otimizando o código como um todo, e não apenas em partes isoladas, o que viabiliza melhorias inter-procedurais. 

#+BEGIN_QUOTE
Além disso, ferramentas como o [[https://github.com/google/autofdo][AutoFDO]] automatizam a coleta de perfis de execução, eliminando a necessidade de instrumentação manual e tornando o processo de otimização mais eficiente. Dessa forma, os compiladores atuais não se limitam a aplicar regras fixas, mas evoluem para sistemas adaptativos, capazes de aprender e se ajustar continuamente com base em dados reais de uso.
#+END_QUOTE
...
A tabela de símbolos é uma estrutura fundamental em compiladores modernos, armazenando informações sobre variáveis, funções e seus atributos, como tipo, escopo e, no caso de funções, parâmetros e tipos de retorno. Em linguagens como [[https://www.typescriptlang.org/][TypeScript]] ou [[https://go.dev/][Go]], que possuem sistemas de tipos avançados, a tabela de símbolos é essencial para suportar inferência de tipos e verificações de escopo em tempo de compilação. Estruturas de dados eficientes, como tabelas de hash ou árvores balanceadas, são usadas para garantir acesso rápido a essas informações.

*** 1.2.9 Agrupamento de Fases em Passos

Na prática, as fases de compilação são frequentemente agrupadas em passos para otimizar o desempenho. Por exemplo, em compiladores como [[https://clang.llvm.org/][Clang]] ou [[https://www.rust-lang.org/][Rustc]], o front-end (análise léxica, sintática, semântica e geração de código intermediário) pode ser combinado em um único passo, enquanto otimizações e geração de código para a máquina alvo formam passos separados. 

O uso de representações intermediárias padronizadas, como a [[https://llvm.org/docs/IR.html][IR do LLVM]], permite criar compiladores modulares, combinando front-ends para diferentes linguagens com back-ends para várias arquiteturas, um modelo amplamente adotado em ferramentas modernas. Essa abordagem reflete a evolução dos compiladores, que hoje lidam com linguagens mais complexas e arquiteturas diversas, mantendo a eficiência e a portabilidade como prioridades.

*** 1.2.10 Ferramentas para Construção de Compilador

No desenvolvimento de compiladores modernos, os projetistas contam com uma ampla gama de ferramentas especializadas que simplificam e aceleram a construção de diferentes fases do compilador. Além de ferramentas genéricas de desenvolvimento de software, como editores de texto avançados (e.g., [[https://code.visualstudio.com/][VS Code]]), sistemas de controle de versão (e.g., [[https://git-scm.com/][Git]]), e depuradores, ferramentas específicas para compiladores têm evoluído significativamente, integrando algoritmos complexos e interfaces que facilitam sua adoção. Essas ferramentas frequentemente utilizam linguagens declarativas ou especificações formais para definir componentes do compilador, permitindo integração fluida com o restante do sistema. As principais ferramentas incluem:

1. /Geradores de Analisadores Sintáticos/: Ferramentas como [[https://www.gnu.org/software/bison/][Bison]] e [[https://www.gnu.org/software/yacc/][Yacc]] geram analisadores sintáticos a partir de gramáticas livres de contexto, descritas em linguagens como BNF (Backus-Naur Form). Essas ferramentas são amplamente usadas em projetos como GCC e Clang para automatizar a construção de parsers.

2. /Geradores de Analisadores Léxicos/: Ferramentas como [[https://github.com/westes/flex][Flex]] e [[https://github.com/westes/flex][Lex]] criam analisadores léxicos com base em expressões regulares que descrevem os tokens de uma linguagem. Elas são essenciais para identificar palavras-chave, identificadores e outros elementos léxicos em linguagens como C++ ou Rust.

3. /Mecanismos de Tradução Dirigida por Sintaxe/: Ferramentas como [[https://www.antlr.org/][ANTLR]] permitem a geração de código intermediário a partir de árvores de derivação, utilizando regras sintáticas anotadas. Elas são amplamente usadas em compiladores modernos para traduzir construções de alto nível em representações intermediárias.
...
5. /Mecanismos de Análise de Fluxo de Dados/: Ferramentas como as integradas ao [[https://llvm.org/][LLVM]] ou ao [[https://gcc.gnu.org/][GCC]] realizam análises de fluxo de dados para rastrear como valores são propagados no programa. Essas análises são fundamentais para otimizações como eliminação de código morto e propagação de constantes.

6. /Conjuntos de Ferramentas para Construção de Compiladores/: Frameworks como [[https://llvm.org/][LLVM]] e [[https://gcc.gnu.org/][GCC]] oferecem um ecossistema integrado de rotinas para todas as fases do compilador, desde a análise léxica até a geração de código. Esses frameworks são amplamente adotados em projetos de compiladores para linguagens como Rust, Swift e WebAssembly.
...
- /Linguagens de Quarta Geração/: Linguagens voltadas para aplicações específicas, como [[https://en.wikipedia.org/wiki/SQL][SQL]] (bancos de dados) e [[https://en.wikipedia.org/wiki/R_(programming_language][R]]) (análise de dados).
- /Linguagens de Quinta Geração/: Linguagens baseadas em lógica, como [[https://en.wikipedia.org/wiki/Prolog][Prolog]], usadas em inteligência artificial.

Além disso, linguagens são classificadas como /imperativas/ (e.g., C++, Java), que manipulam o estado do programa, ou /declarativas/ (e.g., Haskell, Prolog), que especificam o quê deve ser computado sem detalhar o como. Linguagens orientadas a objetos, como [[https://en.wikipedia.org/wiki/Java_(programming_language][Java]]) e [[https://en.wikipedia.org/wiki/Python_(programming_language][Python]]), e linguagens de script, como [[https://en.wikipedia.org/wiki/JavaScript][JavaScript]] e [[https://en.wikipedia.org/wiki/Ruby_(programming_language][Ruby]]), dominam o desenvolvimento moderno devido à sua flexibilidade e produtividade.

-----

*** 1.3.2 Impactos nos Compiladores

O avanço das linguagens de programação e das arquiteturas de hardware impõe desafios constantes aos projetistas de compiladores. Linguagens modernas, como [[https://www.rust-lang.org/][Rust]] (com ênfase em segurança de memória) ou [[https://www.typescriptlang.org/][TypeScript]] (com tipagem estática em JavaScript), exigem compiladores que suportem verificações complexas de tipos e otimizações avançadas. Arquiteturas modernas, como GPUs e processadores multicore, requerem que os compiladores gerem código que explore paralelismo e eficiência energética.

Compiladores como [[https://clang.llvm.org/][Clang]], [[https://www.rust-lang.org/][Rustc]] e o [[https://v8.dev/][V8]] (para JavaScript) minimizam o custo de execução de linguagens de alto nível, permitindo que sejam amplamente adotadas. Além disso, compiladores são usados para avaliar novas arquiteturas antes da fabricação, como em simulações de chips RISC-V. A complexidade dos compiladores modernos, que frequentemente integram múltiplas linguagens e alvos, exige boas práticas de engenharia de software, como modularidade e testes automatizados.
...
O cenário atual do desenvolvimento de linguagens de programação mostra uma demanda crescente por especialistas em compiladores. Novas linguagens, como [[https://www.modular.com/mojo][Mojo]] e [[https://ziglang.org/][Zig]], dependem de compiladores modernos e sofisticados para atingir seus objetivos de performance e segurança, utilizando tecnologias como [[https://mlir.llvm.org/][MLIR]] e [[https://llvm.org/][LLVM]]. 
...
A orientação por objeto foi introduzida inicialmente na linguagem [[https://en.wikipedia.org/wiki/Simula][Simula]] em 1967, e incorporada em linguagens como [[https://en.wikipedia.org/wiki/Smalltalk][Smalltalk]], [[https://en.wikipedia.org/wiki/C%2B%2B][C++]], [[https://en.wikipedia.org/wiki/C_Sharp_(programming_language][C#]]) e [[https://en.wikipedia.org/wiki/Java_(programming_language][Java]]). As principais idéias por trás da orientação por objeto são:
...
O paralelismo moderno foi muito além das antigas arquiteturas [[https://en.wikipedia.org/wiki/Very_long_instruction_word][VLIW]] e, em 2025, está centrado em três grandes pilares: instruções vetoriais (vector/SIMD), GPUs e aceleradores especializados para inteligência artificial. As instruções vetoriais, como [[https://en.wikipedia.org/wiki/RISC-V][RISC-V]] (com suporte completo em GCC/LLVM), [[https://en.wikipedia.org/wiki/ARM_architecture#NEON][ARM NEON]] (presente em todos os smartphones e tablets) e [[https://en.wikipedia.org/wiki/Advanced_Vector_Extensions][x86 AVX-512]] (usado em aplicações científicas), permitem que múltiplos dados sejam processados simultaneamente, acelerando operações numéricas. Compiladores modernos, como GCC, Clang e LLVM, já realizam auto-vectorização, ou seja, transformam automaticamente código sequencial em operações vetoriais para aproveitar ao máximo o hardware disponível.

Além disso, as GPUs se consolidaram como o novo paradigma de computação paralela. Tecnologias como [[https://en.wikipedia.org/wiki/CUDA][CUDA]] (NVIDIA), [[https://en.wikipedia.org/wiki/OpenCL][OpenCL]] (padrão aberto para diferentes tipos de hardware), [[https://en.wikipedia.org/wiki/Vulkan_(API][Vulkan Compute]]) e [[https://en.wikipedia.org/wiki/Metal_(API][Metal]]) (Apple) permitem que programas sejam escritos para explorar milhares de núcleos de processamento em paralelo, acelerando tarefas que vão de gráficos a inteligência artificial. Em paralelo, aceleradores de IA, como as [[https://en.wikipedia.org/wiki/Tensor_Processing_Unit][TPUs]] do Google, [[https://en.wikipedia.org/wiki/Neural_Processing_Unit][NPUs]] presentes em smartphones (Apple Neural Engine, Qualcomm Hexagon), [[https://en.wikipedia.org/wiki/ROCm][AMD ROCm]] e [[https://en.wikipedia.org/wiki/Intel_oneAPI][Intel oneAPI]], oferecem plataformas dedicadas para executar modelos de machine learning com máxima eficiência.

Para tirar proveito desses recursos, surgiram compiladores especializados em IA, como o [[https://tvm.apache.org/][TVM]] (Apache), [[https://www.iree.dev/][IREE]] (Google), [[https://mlir.llvm.org/][MLIR]] e [[https://onnxruntime.ai/][ONNX Runtime]], que otimizam modelos de aprendizado de máquina para diferentes tipos de hardware. O ecossistema [[https://en.wikipedia.org/wiki/RISC-V][RISC-V]], por sua vez, já está presente em placas de desenvolvimento (como [[https://en.wikipedia.org/wiki/Raspberry_Pi_Pico][Raspberry Pi Pico]], [[https://en.wikipedia.org/wiki/ESP32][ESP32-C3]], [[https://en.wikipedia.org/wiki/SiFive][SiFive HiFive]]) e em smartphones (Google Pixel 6, Samsung Exynos) e em servidores de grandes empresas de nuvem (Alibaba Cloud, Tencent Cloud), com toolchains modernos ([[https://gcc.gnu.org/][GCC 12+]], [[https://llvm.org/][LLVM 15+]]) oferecendo suporte completo. Assim, o paralelismo atual é caracterizado pela heterogeneidade e pela capacidade dos compiladores de explorar, de forma automática, o melhor de cada arquitetura.
...
/RISC/: Um dos exemplos mais conhecidos de como os compiladores influenciaram o projeto da arquitetura de computador foi a invenção da arquitetura [[https://en.wikipedia.org/wiki/Reduced_instruction_set_computer][RISC]] (Reduced Instruction-Set Computer – computador com um conjunto reduzido de instruções). 

Antes dessa invenção, a tendência era desenvolver gradativamente conjuntos de instruções cada vez mais complexos, com o objetivo de tornar a programação assembler mais fácil; essas arquiteturas eram conhecidas como [[https://en.wikipedia.org/wiki/Complex_instruction_set_computer][CISC]] (Complex Instruction Set Computer – computador com um conjunto de instruções complexas). Por exemplo, os conjuntos de instruções CISC incluem modos de endereçamento de memória complexos para dar suporte aos acessos a estruturas de dados e instruções de chamada de procedimento que salvam registradores e passam parâmetros na pilha.
