...
O Rust costuma ser apresentado como /a linguagem que impede aqueles bugs de memória cabeludos/ antes mesmo do seu código rodar. Mas essa história não para no /[[https://doc.rust-lang.org/book/ch04-00-understanding-ownership.html#the-borrow-checker][borrow checker]]/: ela se estende à concorrência. O pessoal da comunidade fala em /fearless concurrency/ — “concorrência sem medo”. Mas o que isso significa realmente? 
...
Um exemplo clássico de problema de concorrência aconteceu no [[https://www.kernel.org/][Linux]], documentado no [[https://bugzilla.redhat.com/show_bug.cgi?id=CVE-2022-49443][CVE‑2022‑49443]]. Nesse caso, duas partes diferentes do sistema tentaram acessar e modificar a mesma lista na memória ao mesmo tempo, sem nenhum mecanismo de sincronização para coordenar esse acesso. Como resultado, ocorreu um [[https://en.wikipedia.org/wiki/Data_race][data race]], em que as operações simultâneas causaram inconsistências e corromperam o estado interno da lista. 

O kernel do Linux detectou esse acesso inseguro e emitiu um alerta, mostrando exatamente onde a leitura e a escrita concorrentes aconteceram. Esse tipo de bug é difícil de prever e reproduzir, pois depende do momento exato em que as threads acessam o recurso compartilhado, podendo causar falhas imprevisíveis e difíceis de depurar. Abaixo está o alerta gerado pelo [[https://www.kernel.org/doc/html/latest/dev-tools/kcsan.html][KCSAN]]:
...
Para resolver esse tipo de problema, é preciso adicionar mecanismos de sincronização — como se fosse um "sinal vermelho" — para garantir que apenas uma thread por vez possa acessar ou modificar o recurso compartilhado, evitando a bagunça causada por acessos simultâneos. Ferramentas como o /[[https://www.chromium.org/developers/testing/threadsanitizer-tsan-v2/][ThreadSanitizer (TSan)]]/ e o /[[https://www.kernel.org/doc/html/latest/dev-tools/kcsan.html][KCSAN]]/ ajudam a identificar essas [[https://en.wikipedia.org/wiki/Race_condition][race conditions]] durante os testes, monitorando a execução do programa e apontando exatamente onde ocorreu o acesso inseguro como mostra a imagem abaixo:

![KCSAN alerta]()

No entanto, essas ferramentas só conseguem flagrar o erro se ele realmente acontecer durante os testes; caso contrário, o bug pode passar despercebido e só se manifestar depois que o sistema já estiver em produção, como já ocorreu em projetos conhecidos como [[https://github.com/curl/curl/issues/4915][cURL]] e [[https://github.com/grpc/grpc/issues/21729][gRPC]] onde o problema só foi detectado após subir em produção. Em Rust, olha só o que acontece se você tentar rodar esse código que é um exemplo de um [[https://en.wikipedia.org/wiki/Data_race][data race]]:
...
/ [[https://doc.rust-lang.org/book/ch16-00-concurrency.html][The Rust Programming Language (Rust Book) — /Fearless Concurrency*]] – Capítulo do livro oficial do Rust sobre concorrência segura e paradigmas suportados.
/ [[https://doc.rust-lang.org/reference/behavior-considered-undefined.html#data-races][Rust Reference — /Behavior considered undefined: Data Races/]] – Referência formal: /data race/ em Rust é considerado /undefined behavior* (por isso é proibido em código seguro).
/ [[https://doc.rust-lang.org/std/marker/index.html][Documentação Rust – /Send e Sync* (std::marker)]] – Explicação das marker traits ~Send~ e ~Sync~ na biblioteca padrão (o compilador implementa automaticamente para a maioria dos tipos).
/ [[https://blog.rust-lang.org/2015/04/10/Fearless-Concurrency.html][Aaron Turon – /Fearless Concurrency in Rust/ (Rust Blog, 2015)]] – Post no blog oficial introduzindo o slogan /concorrência sem medo* e discutindo como o modelo de ownership do Rust previne bugs comuns.
/ [[https://doc.rust-lang.org/nomicon/shared-mutatability.html][The Rustonomicon – /Sharing & Mutation*]] – Capítulo do “Rustonomicon” detalhando como Rust lida com mutabilidade compartilhada de forma segura, incluindo o papel de ~UnsafeCell~, ~Send~ e ~Sync~.
/ [[https://rust-lang.github.io/async-book/03_async_await/01_chapter.html][Async in Rust – /Pinning and ~Send~ in Futures*]] – Documentação do Async Book enfatizando que futures precisam ser ~Send~ para uso em executores multi-thread, e como o compilador verifica isso.
/ [[https://go.dev/ref/mem][Go Language Spec – /The Go Memory Model*]] – Documento oficial do Go descrevendo o modelo de memória. Ressalta que data races são erros e que programas sem data race se comportam como se fossem sequenciais (DRF-SC).
/ [[https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#Rconc-avoid-data-races][C++ Core Guidelines – /CP.2: Avoid data races/]] – Guia de melhores práticas de C++: /“Unless you do, nothing is guaranteed to work.”* Discute o perigo extremo de data races em C/C++.
* [[https://bugzilla.redhat.com/show_bug.cgi?id=CVE-2022-49443][2348240 – (CVE-2022-49443) CVE-2022-49443 kernel: list: fix a data-race around ep->rdllist]] – Bugzilla do Red Hat com relatório de data race no kernel Linux.
* [[https://bugzilla.redhat.com/show_bug.cgi?id=CVE-2022-49443&utm_source=chatgpt.com][list: fix a data-race around ep->rdllist - Red Hat Bugzilla]] – Bugzilla do Red Hat com relatório de data race no kernel Linux.
* [[https://www.chromium.org/developers/testing/threadsanitizer-tsan-v2/?utm_source=chatgpt.com][ThreadSanitizer (TSan) v. 2 - The Chromium Projects]] – Documentação do ThreadSanitizer (TSan) v. 2.
* [[https://research.google.com/pubs/archive/35604.pdf?utm_source=chatgpt.com][PDF ThreadSanitizer: data race detection in practice - Google Research]] – Artigo do Google Research sobre detecção de data races com ThreadSanitizer.
* [[https://github.com/curl/curl/issues/4915?utm_source=chatgpt.com][tsan: data race in multi.c when shared connection cache · Issue #4915]] – Issue do GitHub com relatório de data race no cURL.
* [[https://github.com/grpc/grpc/issues/21729?utm_source=chatgpt.com][Data race in C++ example greeter_async_client2 #21729 - GitHub]] – Issue do GitHub com relatório de data race no gRPC.
* [[https://bugzilla.mozilla.org/show_bug.cgi?id=1704227&utm_source=chatgpt.com][CVE-2021-29952) ThreadSanitizer: data race @ mozilla::layers ...]] – Issue do Bugzilla do Mozilla com relatório de data race no Firefox.
* [[https://github.com/ClickHouse/ClickHouse/issues/69520?utm_source=chatgpt.com][Data race in ~WriteBufferFromHTTPServerResponse~ · Issue #69520 ...]] – Issue do GitHub com relatório de data race no ClickHouse.
* [[https://stackoverflow.com/questions/72987598/rust-why-is-rc-not-send-in-the-following-scenario?utm_source=chatgpt.com][Rust - Why is Rc not Send in the following scenario? - Stack Overflow]]
* [[https://github.com/jonhoo/evmap/issues/1][Current implementation is unsound. Segfault and double free are possible with out-of-sync maps from a bad ~PartialEq~ implementation. · Issue #1 · jonhoo/evmap · GitHub]]
